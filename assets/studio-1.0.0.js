...
(() => {
  ...
  const quizTypeLabel = (type) =>
    ({
      knowledge: "Quiz γνώσεων",
      poll: "Δημοσκόπηση",
      personality: "Τεστ προσωπικότητας",
      survey: "Έρευνα",
    })[type] || "Quiz γνώσεων";
  ...
  const quizThemePresets = {
    atelier: {
      label: "Atelier",
      description: "Μαύρο, χρυσό και λιλά με editorial χαρακτήρα.",
      preset: "atelier",
      primary: "#d9bd85",
      secondary: "#b9a7ff",
      page: "#08080a",
    ...
    classic: {
      label: "Κλασικό",
      description: "Καθαρό και ουδέτερο για κάθε είδος quiz.",
      preset: "classic",
      primary: "#4f46e5",
      secondary: "#7c3aed",
      page: "#f4f4f8",
    ...
    contrast: {
      ...
      description: "Μέγιστη αναγνωσιμότητα και WCAG AA αντίθεση.",
      preset: "contrast",
      primary: "#000000",
      secondary: "#404040",
      page: "#ffffff",
    ...
    news: {
      label: "Ειδήσεις",
      description: "Editorial κόκκινο για ειδησεογραφικά quiz.",
      preset: "news",
      primary: "#b91c1c",
      secondary: "#7f1d1d",
      page: "#f8fafc",
    ...
    dark: {
      label: "Σκούρο",
      description: "Σύγχρονο dark theme με μωβ accent.",
      preset: "dark",
      primary: "#c4b5fd",
      secondary: "#818cf8",
      page: "#09090b",
    ...
    midnight: {
      label: "Μεσονύκτιο",
      description: "Βαθύ μπλε με cyan και violet λεπτομέρειες.",
      preset: "midnight",
      primary: "#67e8f9",
      secondary: "#a78bfa",
      page: "#020617",
    ...
    ocean: {
      label: "Ωκεανός",
      description: "Φωτεινό teal με καθαρή εταιρική εμφάνιση.",
      preset: "ocean",
      primary: "#0f766e",
      secondary: "#0369a1",
      page: "#ecfeff",
    ...
    forest: {
      label: "Δάσος",
      description: "Πράσινο, φυσικό και ήρεμο style.",
      preset: "forest",
      primary: "#166534",
      secondary: "#4d7c0f",
      page: "#f0fdf4",
    ...
    sunset: {
      label: "Ηλιοβασίλεμα",
      description: "Ζεστό πορτοκαλί και ροζ για lifestyle περιεχόμενο.",
      preset: "sunset",
      primary: "#c2410c",
      secondary: "#be123c",
      page: "#fff7ed",
    ...
    sports: {
      label: "Αθλητικά",
      description: "Δυναμικό πράσινο και χρυσό.",
      preset: "sports",
      primary: "#065f46",
      secondary: "#d97706",
      page: "#ecfdf5",
    ...
    magazine: {
      label: "Περιοδικό",
      description: "Editorial serif εμφάνιση με διακριτική πολυτέλεια.",
      preset: "magazine",
      primary: "#7c2d12",
      secondary: "#a16207",
      page: "#fffbeb",
    ...
    corporate: {
      label: "Εταιρικό",
      description: "Μπλε και teal για business περιβάλλον.",
      preset: "corporate",
      primary: "#1d4ed8",
      secondary: "#0f766e",
      page: "#f1f5f9",
    ...
    minimal: {
      label: "Μίνιμαλ",
      description: "Ασπρόμαυρο, λιτό και χωρίς περιττά στοιχεία.",
      preset: "minimal",
      primary: "#111827",
      secondary: "#6b7280",
      page: "#ffffff",
   ...
   */
  const preferenceSurface = () =>
    root.closest(".wpqs-front-studio, .wpqs-admin-wrap") || root;
  ...
  const newQuestion = () => ({
    ...
    settings: {
      ...
      numeric_tolerance: 0,
      rating_max: 5,
      multiple_scoring: "exact",
      order_scoring: "exact",
      matching_scoring: "exact",
      text_case_sensitive: false,
      text_ignore_accents: true,
      text_ignore_punctuation: true,
      rating_style: "stars",
      condition: {
        enabled: false,
        match: "all",
    ...
    answers: [
      newAnswer("Απάντηση 1", true, 1),
      newAnswer("Απάντηση 2", false, 0),
    ],
  });
  ...
  const normaliseQuestionCorrectness = (question) => {
    ...
    if (singleCorrectTypes.includes(question.type)) {
      ...
      question.answers.forEach((answer, index) => {
        if (bool(answer.is_correct)) selected = index;
      });
      question.answers.forEach((answer, index) => {
        answer.is_correct = selected >= 0 && index === selected;
        if (answer.is_correct && number(answer.score) <= 0)
          answer.score = Math.max(1, number(question.settings?.points, 1));
      });
    }
    if (question.type === "multiple_answers" || question.type === "open_text") {
      question.answers.forEach((answer) => {
        if (bool(answer.is_correct) && number(answer.score) <= 0)
          answer.score = 1;
      });
    }
    return question;
  ...
  const normaliseQuizCorrectness = (quiz) => {
    if (quiz && Array.isArray(quiz.questions))
      quiz.questions.forEach(normaliseQuestionCorrectness);
    return quiz;
  };
  ...
  const configureQuestionType = (question, nextType) => {
    ...
    question.type = questionTypes.includes(nextType)
      ? nextType
      : "multiple_choice";
    question.answers = Array.isArray(question.answers) ? question.answers : [];

    const ensureAnswers = (minimum = 2) => {
      while (question.answers.length < minimum)
        question.answers.push(
          newAnswer(`Απάντηση ${question.answers.length + 1}`, false, 0),
        );
    };
    ...
    if (question.type === "true_false") {
      if (previousType !== "true_false") {
        question.answers = [
          newAnswer("Σωστό", true, 1),
          newAnswer("Λάθος", false, 0),
        ];
      } else {
        ensureAnswers(2);
      ...
      ensureAnswers(2);
      if (!question.answers.some((answer) => bool(answer.is_correct)))
      ...
      question.answers.forEach((answer, index) => {
        answer.is_correct = false;
        if (question.type === "matching" && !answer.content.match_text)
          answer.content.match_text = `Αντιστοίχιση ${index + 1}`;
      });
      ...
      if (question.type === "rating") {
        question.settings.rating_max = Math.max(
          2,
          number(question.settings.rating_max, 5),
        );
        question.settings.slider_min = 1;
        question.settings.slider_max = question.settings.rating_max;
        question.settings.correct_min = Math.max(
          1,
          number(question.settings.correct_min, 1),
        );
        question.settings.correct_max = Math.min(
          question.settings.rating_max,
          number(question.settings.correct_max, question.settings.rating_max),
        );
      }
    }
  ...
  const api = async (path, options = {}) => {
    ...
    try {
      ...
      throw new Error(
        `Ο διακομιστής επέστρεψε μη έγκυρη απάντηση (${response.status}) για ${path}.`,
      );
    }
    if (!response.ok) {
      const error = new Error(
        data?.message || `Το αίτημα απέτυχε (${response.status}).`,
      );
  ...
  const emptyQuiz = () => ({
    ...
    organization_id: number(WPQS.context?.organization_id),
    template_id: 0,
    review_comment: "",
    scheduled_at: null,
    expires_at: null,
    ...
    settings: {
      ...
      personality_tie_strategy: "first",
      results: [],
      embed_mode: "inherit",
      embed_domains: [],
      embed_block_message: "",
  ...
  const normaliseQuiz = (raw) => {
    ...
    quiz.quiz_type = quizTypes.includes(quiz.quiz_type)
      ? quiz.quiz_type
      : "knowledge";
    quiz.status = [
      "draft",
    ...
    quiz.workflow_status = [
      ...
      "published",
      "archived",
    ].includes(quiz.workflow_status)
      ? quiz.workflow_status
      : quiz.status === "published"
        ? "published"
        : "draft";
    quiz.visibility_scope = ["personal", "organization", "universal"].includes(
      quiz.visibility_scope,
    )
      ? quiz.visibility_scope
      : "personal";
    quiz.organization_id = number(
      quiz.organization_id || WPQS.context?.organization_id,
    );
    ...
    quiz.review_history = Array.isArray(quiz.review_history)
      ? quiz.review_history
      : [];
    quiz.scheduled_at =
      typeof quiz.scheduled_at === "string" && quiz.scheduled_at
        ? quiz.scheduled_at
        : null;
    quiz.expires_at =
      typeof quiz.expires_at === "string" && quiz.expires_at
        ? quiz.expires_at
        : null;
    quiz.category_id = number(quiz.category_id);
    quiz.category =
      quiz.category && typeof quiz.category === "object" ? quiz.category : null;
    quiz.settings =
      quiz.settings && typeof quiz.settings === "object" ? quiz.settings : {};
    quiz.settings.intro =
      quiz.settings.intro && typeof quiz.settings.intro === "object"
        ? quiz.settings.intro
        : {};
    quiz.settings.intro = {
      ...defaults.settings.intro,
      ...quiz.settings.intro,
    };
    quiz.settings.category =
      typeof quiz.settings.category === "string" ? quiz.settings.category : "";
    quiz.settings.show_progress =
      quiz.settings.show_progress === undefined
        ? true
        : bool(quiz.settings.show_progress);
    quiz.settings.random_questions = bool(quiz.settings.random_questions);
    quiz.settings.random_question_limit = number(
      quiz.settings.random_question_limit,
    );
    quiz.settings.show_feedback =
      quiz.settings.show_feedback === undefined
        ? true
        : bool(quiz.settings.show_feedback);
    quiz.settings.show_correct_answer =
      quiz.settings.show_correct_answer === undefined
        ? true
        : bool(quiz.settings.show_correct_answer);
    quiz.settings.allow_restart =
      quiz.settings.allow_restart === undefined
        ? true
        : bool(quiz.settings.allow_restart);
    quiz.settings.review_answers =
      quiz.settings.review_answers === undefined
        ? true
        : bool(quiz.settings.review_answers);
    quiz.settings.show_pass_fail = bool(quiz.settings.show_pass_fail);
    quiz.settings.pass_score = number(quiz.settings.pass_score);
    quiz.settings.embed_mode = ["inherit", "public", "restricted"].includes(
      quiz.settings.embed_mode,
    )
      ? quiz.settings.embed_mode
      : "inherit";
    quiz.settings.embed_domains = Array.isArray(quiz.settings.embed_domains)
      ? quiz.settings.embed_domains.map(String)
      : typeof quiz.settings.embed_domains === "string"
        ? quiz.settings.embed_domains.split(/[\r\n,;]+/).filter(Boolean)
        : [];
    quiz.settings.embed_block_message = String(
      quiz.settings.embed_block_message || "",
    );
    quiz.settings.personality_tie_strategy = ["first", "all"].includes(
      quiz.settings.personality_tie_strategy,
    )
      ? quiz.settings.personality_tie_strategy
      : "first";
    quiz.settings.personality_profiles = Array.isArray(
      quiz.settings.personality_profiles,
    )
      ? quiz.settings.personality_profiles.map((profile) => ({
          key: String(profile.key || makeKey("profile")),
          title: String(profile.title || "Προσωπικότητα"),
          description: String(profile.description || ""),
          image_id: number(profile.image_id),
          image_url: String(profile.image_url || ""),
          cta_label: String(profile.cta_label || ""),
          cta_url: String(profile.cta_url || ""),
        }))
      : [];
    quiz.settings.results = Array.isArray(quiz.settings.results)
      ? quiz.settings.results.map((range) => ({
          min: number(range.min),
          max: number(range.max),
          title: String(range.title || "Αποτέλεσμα"),
          description: String(range.description || ""),
          image_id: number(range.image_id),
    ...
    quiz.theme = {
      ...defaults.theme,
      ...(quiz.theme && typeof quiz.theme === "object" ? quiz.theme : {}),
    };
    quiz.questions = Array.isArray(quiz.questions)
      ? quiz.questions.map((rawQuestion) => {
          ...
          question.type = questionTypes.includes(question.type)
            ? question.type
            : "multiple_choice";
          question.content =
            question.content && typeof question.content === "object"
              ...
              : {};
          question.content = {
            title: String(question.content.title || ""),
            image_id: number(question.content.image_id),
            image_url: String(question.content.image_url || ""),
            video_url: String(question.content.video_url || ""),
          ...
          const condition =
            question.settings.condition &&
            typeof question.settings.condition === "object"
              ? question.settings.condition
              : {};
          let rules = Array.isArray(condition.rules) ? condition.rules : [];
          if (!rules.length && condition.question_key)
            rules = [
              {
                operator: condition.operator || "equals",
                question_key: condition.question_key,
                answer_key: condition.answer_key || "",
              },
            ];
          rules = rules.map((rule) => ({
            operator: [
              ...
              ? rule.operator
              : "equals",
            question_key: String(rule.question_key || ""),
            answer_key: String(rule.answer_key || ""),
          }));
          question.settings = {
            key: String(question.settings.key || makeKey("q")),
            hint: String(question.settings.hint || ""),
            explanation: String(question.settings.explanation || ""),
            points: number(question.settings.points, 1),
            timer: number(question.settings.timer),
            shuffle_answers: bool(question.settings.shuffle_answers),
            required:
              question.settings.required === undefined
                ? true
                : bool(question.settings.required),
            slider_min: number(question.settings.slider_min),
            slider_max: number(question.settings.slider_max, 100),
            slider_step: number(question.settings.slider_step, 1),
            correct_min: number(question.settings.correct_min),
            correct_max: number(question.settings.correct_max, 100),
            numeric_answer: number(question.settings.numeric_answer),
            numeric_tolerance: number(question.settings.numeric_tolerance),
            rating_max: number(question.settings.rating_max, 5),
            multiple_scoring: ["exact", "partial"].includes(
              question.settings.multiple_scoring,
            )
              ? question.settings.multiple_scoring
              : "exact",
            order_scoring: ["exact", "partial"].includes(
              question.settings.order_scoring,
            )
              ? question.settings.order_scoring
              : "exact",
            matching_scoring: ["exact", "partial"].includes(
              question.settings.matching_scoring,
            )
              ? question.settings.matching_scoring
              : "exact",
            text_case_sensitive: bool(question.settings.text_case_sensitive),
            text_ignore_accents:
              question.settings.text_ignore_accents === undefined
                ? true
                : bool(question.settings.text_ignore_accents),
            text_ignore_punctuation:
              question.settings.text_ignore_punctuation === undefined
                ? true
                : bool(question.settings.text_ignore_punctuation),
            rating_style: ["stars", "numbers"].includes(
              question.settings.rating_style,
            )
              ? question.settings.rating_style
              : "stars",
            condition: {
              enabled: bool(condition.enabled),
              match: ["all", "any"].includes(condition.match)
                ? condition.match
                : "all",
              rules,
              operator: rules[0]?.operator || "equals",
              question_key: rules[0]?.question_key || "",
              answer_key: rules[0]?.answer_key || "",
            },
          };
          question.answers = Array.isArray(question.answers)
            ? question.answers.map((rawAnswer) => {
                const answer =
                  rawAnswer && typeof rawAnswer === "object" ? rawAnswer : {};
                answer.content =
                  answer.content && typeof answer.content === "object"
                    ? answer.content
                    : {};
                answer.content = {
                  key: String(answer.content.key || makeKey("a")),
                  text: String(answer.content.text || ""),
                  match_text: String(answer.content.match_text || ""),
                  image_id: number(answer.content.image_id),
                  image_url: String(answer.content.image_url || ""),
                  emoji: String(answer.content.emoji || ""),
                  icon: String(answer.content.icon || ""),
                  personality_weights:
                    answer.content.personality_weights &&
                    typeof answer.content.personality_weights === "object"
                      ? { ...answer.content.personality_weights }
                      : {},
                };
  ...
  let state = {
    view: "list",
    portalView: window.WPQS?.initialTab || "dashboard",
    tab: "questions",
    quizzes: [],
    quiz: null,
    ...
    listWorkflow: "all",
    listCreator: "all",
    listSort: localStorage.getItem("wpqs_list_sort") || "updated_desc",
    listView: localStorage.getItem("wpqs_list_view") || "grid",
    analyticsPreset: "30",
    analyticsFrom: "",
    analyticsTo: "",
    analyticsGroup: "day",
    userPreferences: normaliseUserPreferences(WPQS.userPreferences),
    categoryQuery: "",
    userWorkspaceQuery: "",
  ...
  const recoveryKey = (quiz) =>
    `quiz_atelier_recovery_${number(WPQS.context?.user_id || state.me?.id)}_${number(quiz?.id) || "new"}`;
  const storeRecovery = () => {
    if (!state.quiz || state.view !== "builder") return;
    try {
      localStorage.setItem(
        recoveryKey(state.quiz),
        JSON.stringify({ saved_at: Date.now(), quiz: state.quiz }),
      );
    } catch (_) {}
  };
  const removeRecovery = (quiz) => {
    try {
      localStorage.removeItem(recoveryKey(quiz));
    } catch (_) {}
  };
  const recoverQuiz = (quiz) => {
    try {
      ...
      const serverTime = quiz.updated_at
        ? new Date(String(quiz.updated_at).replace(" ", "T") + "Z").getTime()
        : 0;
      if (!recovery?.quiz || number(recovery.saved_at) <= serverTime) {
        removeRecovery(quiz);
        return quiz;
      }
      if (
        confirm(
          "Βρέθηκε νεότερη τοπική αυτόματη ανάκτηση για αυτό το quiz. Να επαναφερθεί;",
        )
      )
        return normaliseQuiz(recovery.quiz);
      removeRecovery(quiz);
    } catch (_) {}
    return quiz;
  };
  ...
  const preserveBuilderPosition = (callback, focusSelector = "") => {
    ...
    const anchorSelector =
      focusSelector ||
      (activeCard
        ? `[data-question-card="${activeCard.dataset.questionCard}"]`
        : "");
    ...
    requestAnimationFrame(() =>
      requestAnimationFrame(() => {
        ...
        const nextAnchor = anchorSelector
          ? root.querySelector(anchorSelector)
          : null;
        if (anchorTop !== null && nextAnchor) {
          window.scrollBy({
            left: 0,
            top: nextAnchor.getBoundingClientRect().top - anchorTop,
            behavior: "auto",
          });
        } else {
        ...
        if (focusSelector) {
          ...
          if (
            ...
            !["SELECT", "BUTTON"].includes(field.tagName)
          ) {
  ...
  const collectValidationIssues = (quiz, publishing = false) => {
    const issues = [];
    const push = (message, questionIndex = null, selector = "") =>
      issues.push({ message, questionIndex, selector });
    if (!String(quiz.title || "").trim())
      push("Συμπληρώστε τον τίτλο του quiz.", null, '[data-field="title"]');
    if (!publishing) return issues;
    if (!Array.isArray(quiz.questions) || !quiz.questions.length)
      push(
        "Προσθέστε τουλάχιστον μία ερώτηση πριν από τη δημοσίευση.",
        null,
        "[data-add-question]",
      );
    ...
    (quiz.questions || []).forEach((question, index) => {
      ...
      if (!String(question.content?.title || "").trim())
        push(
          `${label}: λείπει το κείμενο της ερώτησης.`,
          index,
          `[data-question-title="${index}"]`,
        );
      ...
      if (answerTypes.includes(question.type) && answers.length < minimum)
        push(
          `${label}: χρειάζονται τουλάχιστον ${minimum} ${minimum === 1 ? "απάντηση" : "απαντήσεις"}.`,
          index,
          selector,
        );
      answers.forEach((answer, answerIndex) => {
        if (
          answerTypes.includes(question.type) &&
          !String(answer.content?.text || "").trim()
        )
          push(
            `${label}: η απάντηση ${answerIndex + 1} είναι κενή.`,
            index,
            `[data-answer-question="${index}"][data-answer="${answerIndex}"]`,
          );
        if (
          question.type === "image_choice" &&
          !String(answer.content?.image_url || "").trim()
        )
          push(
            `${label}: η απάντηση ${answerIndex + 1} χρειάζεται εικόνα.`,
            index,
            selector,
          );
        if (
          question.type === "matching" &&
          !String(answer.content?.match_text || "").trim()
        )
          push(
            `${label}: λείπει η δεξιά τιμή στο ζεύγος ${answerIndex + 1}.`,
            index,
            `[data-answer-match][data-question="${index}"][data-answer="${answerIndex}"]`,
          );
      });

      if (question.type === "true_false" && answers.length !== 2)
        push(
          `${label}: ο τύπος Σωστό / Λάθος πρέπει να έχει ακριβώς δύο επιλογές.`,
          index,
          selector,
        );
      if (
        quiz.quiz_type !== "personality" &&
        ![
          "poll",
          "ordering",
          "ranking",
        ...
        if (
          ["multiple_choice", "true_false", "image_choice"].includes(
            question.type,
          ) &&
          correct !== 1
        )
          push(
            `${label}: επιλέξτε ακριβώς μία σωστή απάντηση.`,
            index,
            selector,
          );
        if (
          ["multiple_answers", "open_text"].includes(question.type) &&
          correct < 1
        )
          push(
            `${label}: επιλέξτε τουλάχιστον μία σωστή ή αποδεκτή απάντηση.`,
            index,
            selector,
          );
      }
      if (
        ...
        !Number.isFinite(Number(question.settings?.numeric_answer))
      )
        push(
          `${label}: ορίστε έγκυρη αριθμητική απάντηση.`,
          index,
          `[data-question-setting="numeric_answer"][data-index="${index}"]`,
        );
      if (question.type === "slider") {
        const min = number(question.settings?.slider_min);
        const max = number(question.settings?.slider_max);
        const step = number(question.settings?.slider_step);
        if (max <= min)
          push(
            `${label}: το μέγιστο της κλίμακας πρέπει να είναι μεγαλύτερο από το ελάχιστο.`,
            index,
            `[data-question-setting="slider_max"][data-index="${index}"]`,
          );
        if (step <= 0)
          push(
            `${label}: το βήμα της κλίμακας πρέπει να είναι μεγαλύτερο από μηδέν.`,
            index,
            `[data-question-setting="slider_step"][data-index="${index}"]`,
          );
        if (
          number(question.settings?.correct_min) < min ||
          number(question.settings?.correct_max) > max ||
          number(question.settings?.correct_min) >
            number(question.settings?.correct_max)
        )
          push(
            `${label}: το σωστό εύρος πρέπει να βρίσκεται μέσα στα όρια της κλίμακας.`,
            index,
            selector,
          );
      }
      if (question.type === "rating") {
        ...
        if (ratingMax < 2 || ratingMax > 20)
          push(
            `${label}: η μέγιστη αξιολόγηση πρέπει να είναι από 2 έως 20.`,
            index,
            `[data-question-setting="rating_max"][data-index="${index}"]`,
          );
        if (
          number(question.settings?.correct_min) < 1 ||
          number(question.settings?.correct_max) > ratingMax ||
          number(question.settings?.correct_min) >
            number(question.settings?.correct_max)
        )
          push(
            `${label}: το σωστό εύρος αξιολόγησης δεν είναι έγκυρο.`,
            index,
            selector,
          );
      }
      if (question.settings?.condition?.enabled) {
        const previousKeys = new Set(
          (quiz.questions || [])
            .slice(0, index)
            .map((item) => String(item.settings?.key || "")),
        );
        const rules = Array.isArray(question.settings.condition.rules)
          ? question.settings.condition.rules
          : [];
        if (!rules.length)
          push(
            `${label}: ενεργοποιήθηκαν όροι εμφάνισης χωρίς κανόνα.`,
            index,
            selector,
          );
        rules.forEach((rule) => {
          if (!previousKeys.has(String(rule.question_key || "")))
            push(
              `${label}: ένας κανόνας εμφάνισης αναφέρεται σε μη έγκυρη προηγούμενη ερώτηση.`,
              index,
              selector,
            );
        });
    ...
    if (
      quiz.quiz_type === "personality" &&
      (quiz.settings?.personality_profiles || []).length < 2
    )
      push(
        "Το Personality Test χρειάζεται τουλάχιστον δύο προφίλ αποτελέσματος.",
        null,
        '[data-tab="results"]',
      );
    return issues.filter(
      (issue, index, all) =>
        all.findIndex(
          (item) =>
            item.message === issue.message && item.selector === issue.selector,
        ) === index,
    );
  };

  const validateQuiz = (quiz, publishing = false) =>
    collectValidationIssues(quiz, publishing).map((issue) => issue.message);
  ...
  const load = async () => {
    try {
      ...
      state.quizzes = (Array.isArray(base[0]) ? base[0] : []).map(
        normaliseQuiz,
      );
      ...
      const orgId = number(
        state.me?.context?.organization_id || WPQS.context?.organization_id,
      );
      ...
      if (WPQS.canManageOrganizations)
        state.organizations = await api("organizations");
      if (WPQS.canManageUserWorkspaces) {
        try {
          state.userWorkspaces = await api("admin/user-workspaces");
        } catch (_) {
  ...
  const copyText = async (value) => {
    try {
      ...
      alert(
        "Δεν ήταν δυνατή η αντιγραφή. Επιλέξτε τον κώδικα και αντιγράψτε τον χειροκίνητα.",
      );
    }
  };
  ...
  const openEmbedModal = (quiz) => {
    ...
    const update = () => {
      const height = Math.max(
        ...
        Math.min(
          1800,
          number(modal.querySelector("[data-embed-height]").value, 720),
        ),
      );
      ...
      const radius = Math.max(
        ...
        Math.min(
          80,
          number(modal.querySelector("[data-embed-radius]").value, 0),
        ),
      );
      const background =
        modal.querySelector("[data-embed-background]").value || "#08080a";
      const loading = modal.querySelector("[data-embed-loading]").value;
      const responsive =
        modal.querySelector("[data-embed-responsive]").value === "yes";
      ...
      if (mode === "javascript") {
        ...
        help.innerHTML =
          "Προσθέστε το div και το script σε Custom HTML. Το loader δημιουργεί ασφαλές iframe και εφαρμόζει τις ρυθμίσεις.";
      } else if (mode === "shortcode") {
        code = `[wp_quiz_studio id="${quiz.id}"]`;
        help.innerHTML =
          "<strong>Gutenberg:</strong> προσθέστε block «Shortcode» και επικολλήστε τον κώδικα. <strong>Elementor:</strong> χρησιμοποιήστε widget Shortcode.";
      } else if (mode === "drupal") {
        ...
        help.innerHTML =
          "<strong>Drupal:</strong> χρησιμοποιήστε text format που επιτρέπει iframe ή Custom Block → Full HTML. Προσθέστε το domain στη whitelist του quiz.";
      } else if (mode === "direct") {
        code = directUrl;
        help.innerHTML =
          "Ο direct σύνδεσμος ανοίγει το quiz σε ξεχωριστή σελίδα χωρίς WordPress admin chrome.";
      } else {
        code = `<iframe src="${embedUrl}" width="${width}" height="${height}" frameborder="0" loading="${loading}"${titleAttr} style="${iframeStyle}"></iframe>`;
        help.innerHTML =
          "Το iframe λειτουργεί σε WordPress Custom HTML, Drupal Full HTML, Joomla, Laravel και απλή HTML.";
      }
    ...
    modal.querySelectorAll("[data-embed-mode]").forEach(
      (button) =>
        (button.onclick = () => {
          mode = button.dataset.embedMode;
          modal
            .querySelectorAll("[data-embed-mode]")
            .forEach((item) =>
    ...
    modal
      .querySelectorAll(
        "[data-embed-options] input,[data-embed-options] select",
      )
      .forEach((input) =>
    ...
    modal.querySelector("[data-copy-embed]").onclick = () =>
      copyText(codeField.value);
    modal.querySelector("[data-open-external]").onclick = () =>
  ...
  const openUserStyleModal = () => {
    ...
    const presetOptions = [
      ...
      .map(
        ([value, label]) =>
          `<option value="${value}" ${draft.preset === value ? "selected" : ""}>${label}</option>`,
      )
      .join("");
    const styleColor = (key, label) =>
      `<label>${label}<span class="color-control"><input type="color" data-user-style="${key}" value="${esc(draft[key])}"><input data-user-style-text="${key}" value="${esc(draft[key])}"></span></label>`;
    ...
    const updateStylePreview = () => {
      ...
      if (preview)
        preview.style.cssText = `--style-accent:${draft.accent};--style-accent-text:${draft.accent_text || "#111111"};--style-page:${draft.page};--style-surface:${draft.surface};--style-raised:${draft.surface_raised};--style-text:${draft.text};--style-muted:${draft.muted};--style-border:${draft.border};--style-radius:${number(draft.radius, 18)}px`;
    ...
    modal.querySelector("[data-user-preset]").onchange = (event) => {
      ...
      if (studioStylePresets[selected])
        draft = clone(studioStylePresets[selected]);
    ...
    modal.querySelectorAll("[data-user-style]").forEach((input) => {
      ...
      input.addEventListener(
        input.type === "color" || input.type === "range" ? "input" : "change",
        handler,
      );
    });
    modal.querySelectorAll("[data-user-style-text]").forEach(
      (input) =>
    ...
    modal.querySelector("[data-user-auto-contrast]").onclick = () => {
      ...
      const black = "#111111";
      draft.text =
        contrastRatio(white, draft.surface) >=
        contrastRatio(black, draft.surface)
          ? white
          : black;
      draft.muted = draft.text === white ? "#d4d4d8" : "#374151";
      draft.accent_light =
        contrastRatio(white, draft.accent) >= 4.5 ? white : draft.accent;
      draft.accent_text =
        contrastRatio(white, draft.accent) >= contrastRatio(black, draft.accent)
          ? white
          : black;
      draft.preset = "custom";
      updateFields();
    ...
    modal.querySelector("[data-user-style-reset]").onclick = () => {
      draft = clone(studioStylePresets.atelier);
      updateFields();
      updateStylePreview();
    };
    modal.querySelector("[data-user-style-save]").onclick = async () => {
      ...
      try {
        state.userPreferences = normaliseUserPreferences(
  ...
  const portalIcon = (name) => {
    const paths = {
      ...
      library:
        '<path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20V4H6.5A2.5 2.5 0 0 0 4 6.5z"/><path d="M8 7h8M8 11h8"/>',
      templates:
        '<path d="m12 2 8 4-8 4-8-4z"/><path d="m4 10 8 4 8-4M4 14l8 4 8-4"/>',
      ...
      organizations:
        '<path d="M4 21V8l8-5 8 5v13M8 21v-5h8v5M8 10h1M12 10h1M16 10h1"/>',
      workspace: '<path d="M3 6h18v14H3zM7 3h10v3M7 11h10M7 15h6"/>',
      users:
        '<circle cx="8" cy="8" r="3"/><circle cx="17" cy="7" r="2.5"/><path d="M2.5 20a5.5 5.5 0 0 1 11 0M14 13h7M17.5 9.5v7"/>',
      documentation:
  ...
  const portalNav = (active) => {
    const links = [
      ...
      ["library", "library", "Quiz Library"],
      ["templates", "templates", "Templates"],
      ["categories", "categories", "Κατηγορίες"],
    ];
    if (WPQS.canAnalytics)
      links.push(["analytics-global", "analytics", "Analytics"]);
    if (WPQS.canManageTeam)
      links.push(
        ["workspace", "workspace", "Το Workspace μου"],
        ["team", "team", "Η ομάδα μου"],
        ["activity", "activity", "Activity Log"],
      );
    if (WPQS.canManageUserWorkspaces)
      links.push(
        ["user-workspaces", "users", "Χρήστες & Workspaces"],
        ["system", "system", "System Status"],
      );
    if (WPQS.canManageOrganizations)
      links.push(["organizations", "organizations", "Organizations"]);
    links.push(
      ["documentation", "documentation", "Τεκμηρίωση"],
      ["profile", "profile", "Ο λογαριασμός μου"],
    );
    const frontClass = WPQS.isFront ? " is-front" : "";
    const brand = WPQS.isFront
      ? ""
      : '<div class="wpqs-portal-brand"><b>Quiz</b><span>ATELIER</span></div>';
    const account = WPQS.isFront
      ? ""
      : `<div class="wpqs-portal-account"><span>${esc(state.me?.display_name || WPQS.userName || "")}</span><button type="button" data-user-style-open title="Το στυλ μου" aria-label="Το στυλ μου">◐</button></div>`;
    const buttons = links
      .map(
        ([view, icon, label]) =>
          `<button type="button" class="${active === view ? "is-active" : ""}" data-portal-view="${view}"><i>${portalIcon(icon)}</i><span>${esc(label)}</span></button>`,
      )
      .join("");
  ...
  const bindPortalNav = () => {
    ...
    root
      ...
      .forEach(
        (button) =>
          (button.onclick = () => routePortal(button.dataset.portalView)),
      );
    root
      .querySelectorAll("[data-user-style-open]")
      .forEach((button) =>
        button.addEventListener("click", openUserStyleModal),
      );
  ...
  const renderDashboard = () => {
    ...
    bindPortalNav();
    root.querySelector("[data-dashboard-new]").onclick = () =>
      openQuiz(emptyQuiz());
    root.querySelector("[data-dashboard-templates]").onclick = () =>
      routePortal("templates");
    root.querySelector("[data-dashboard-library]").onclick = () =>
      routePortal("library");
    root
      .querySelector("[data-dashboard-team]")
      ?.addEventListener("click", () => routePortal("team"));
    root
      .querySelector("[data-dashboard-activity]")
      ?.addEventListener("click", () => routePortal("activity"));
    root
      .querySelectorAll("[data-dashboard-edit]")
      .forEach(
        (button) =>
          (button.onclick = async () =>
            openQuiz(await api(`quizzes/${button.dataset.dashboardEdit}`))),
      );
  };

  const templateCard = (template) =>
    `<article class="wpqs-template-card"><div class="wpqs-template-art ${esc(template.quiz_type)}">${template.thumbnail_url ? `<img src="${esc(template.thumbnail_url)}" alt="">` : `<span>${esc(quizTypeLabel(template.quiz_type))}</span><b>Q</b>`}</div><div><span class="wpqs-scope-badge ${esc(template.scope)}">${template.scope === "universal" ? "Universal" : "Organization"}</span><h3>${esc(template.title)}</h3><p>${esc(template.description || "")}</p><div><button class="wpqs-primary" data-template-use="${template.id}">Χρήση template</button><button data-template-preview="${template.id}">Preview</button>${WPQS.canManageUniversal || template.scope !== "universal" ? `<button class="danger" data-template-delete="${template.id}">Διαγραφή</button>` : ""}</div></div></article>`;
  ...
  const renderTemplates = () => {
    ...
    bindPortalNav();
    root.querySelectorAll("[data-template-use]").forEach(
      (button) =>
        (button.onclick = async () => {
          ...
          const quiz = normaliseQuiz({
            ...
            id: undefined,
            title: `${template.title} — Νέο`,
            slug: "",
            status: "draft",
            workflow_status: "draft",
            visibility_scope: "personal",
            organization_id: number(state.me?.context?.organization_id),
    ...
    root.querySelectorAll("[data-template-preview]").forEach(
      (button) =>
        (button.onclick = async () => {
          const template = await api(
            `templates/${button.dataset.templatePreview}`,
          );
          alert(
    ...
    root.querySelectorAll("[data-template-delete]").forEach(
      (button) =>
        (button.onclick = async () => {
          if (!confirm("Να διαγραφεί το template;")) return;
          await api(`templates/${button.dataset.templateDelete}`, {
            method: "DELETE",
          });
  ...
  const activityLabel = (action) =>
    ({
      quiz_created: "Δημιουργία quiz",
      quiz_updated: "Επεξεργασία quiz",
      quiz_deleted: "Διαγραφή quiz",
      quiz_submitted: "Υποβολή για έλεγχο",
      quiz_changes_requested: "Ζητήθηκαν αλλαγές",
      quiz_approved: "Έγκριση quiz",
      quiz_published: "Δημοσίευση quiz",
      member_added: "Προσθήκη μέλους",
      member_updated: "Αλλαγή μέλους",
      member_removed: "Αφαίρεση μέλους",
      invitation_sent: "Αποστολή πρόσκλησης",
      invitation_resent: "Επαναποστολή πρόσκλησης",
      invitation_revoked: "Ανάκληση πρόσκλησης",
      template_created: "Δημιουργία template",
      organization_created: "Δημιουργία Organization",
      organization_updated: "Ενημέρωση Organization",
      workspace_updated: "Ενημέρωση Workspace",
      user_workspace_changed: "Αλλαγή Workspace χρήστη",
      profile_updated: "Ενημέρωση profile",
    })[action] || action.replaceAll("_", " ");
  const activityRows = (rows) =>
    `<div class="wpqs-activity-list">${(rows || []).map((item) => `<article><i></i><div><strong>${esc(activityLabel(item.action))}</strong><p>${esc(item.display_name || "System")} · ${esc(item.object_type)}${item.object_id ? ` #${item.object_id}` : ""}</p></div><time>${esc(formatDate(item.created_at))}</time></article>`).join("") || '<div class="empty-panel">Δεν υπάρχει καταγεγραμμένη δραστηριότητα.</div>'}</div>`;

  const renderActivity = async () => {
    state.view = "activity";
    if (!state.activity.length && WPQS.canManageTeam) {
      try {
        state.activity = await api("activity");
      } catch (_) {}
    }
  ...
  const teamMemberRow = (member) => {
    const protectedAdministrator =
      bool(member.is_wordpress_admin) && !bool(WPQS.context?.is_super_admin);
    const disabled = protectedAdministrator
      ? 'disabled aria-disabled="true"'
      : "";
    const roleBadge = bool(member.is_wordpress_admin)
      ? '<span class="wpqs-protected-badge">WordPress Administrator</span>'
      : "";
  ...
  const renderTeam = async () => {
    ...
    root.querySelector("[data-invite-csv-load]").onclick = async () => {
      ...
      root.querySelector("[data-invite-emails]").value = [
        ...new Set(emails),
      ].join("\n");
    ...
    root.querySelector("[data-send-invitations]").onclick = async () => {
      ...
      try {
        const result = await api(`organizations/${orgId}/invitations`, {
          method: "POST",
          body: JSON.stringify({
            emails,
            org_role: root.querySelector("[data-invite-role]").value,
            expires_days: number(
              root.querySelector("[data-invite-days]").value,
              7,
            ),
          }),
        });
    ...
    root.querySelectorAll("[data-resend-invite]").forEach(
      (button) =>
    ...
    root.querySelectorAll("[data-member-save]").forEach(
      (button) =>
        (button.onclick = async () => {
          const id = button.dataset.memberSave;
          await api(`organizations/${orgId}/members/${id}`, {
            method: "PUT",
            body: JSON.stringify({
              org_role: root.querySelector(`[data-member-role="${id}"]`).value,
              status: root.querySelector(`[data-member-status="${id}"]`).value,
            }),
          });
          state.team = await api(`organizations/${orgId}/members`);
          renderTeam();
    ...
    root.querySelectorAll("[data-member-remove]").forEach(
      (button) =>
        (button.onclick = async () => {
          if (!confirm("Να αφαιρεθεί το μέλος; Τα quiz του δεν θα διαγραφούν."))
            return;
          await api(
            `organizations/${orgId}/members/${button.dataset.memberRemove}`,
            { method: "DELETE", body: "{}" },
          );
          state.team = await api(`organizations/${orgId}/members`);
          renderTeam();
    ...
    root.querySelectorAll("[data-revoke-invite]").forEach(
      (button) =>
        (button.onclick = async () => {
          await api(
            `organizations/${orgId}/invitations/${button.dataset.revokeInvite}`,
            { method: "DELETE" },
          );
          state.team = await api(`organizations/${orgId}/members`);
          renderTeam();
  ...
  const renderWorkspace = async () => {
    ...
    const emailDomains = domains
      .filter((item) => ["email", "both"].includes(item.domain_type))
      .map((item) => item.domain);
    const embedDomains = domains
      .filter((item) => ["embed", "both", "custom"].includes(item.domain_type))
      .map((item) => item.domain);
    ...
    const seatPercent = Math.min(
      100,
      Math.round((seatsUsed / Math.max(1, number(org.seat_limit, 1))) * 100),
    );
    ...
    root.innerHTML = `<main class="wpqs-dashboard wpqs-portal-page wpqs-workspace-page">${portalNav("workspace")}
      ...
      <div class="wpqs-workspace-layout"><section class="panel wpqs-workspace-main"><div class="section-heading"><div><h2>Ταυτότητα Workspace</h2><p>${isSuper ? "Βασικές πληροφορίες, όρια και κύκλος ζωής του οργανισμού." : "Συνοπτικές πληροφορίες του οργανισμού σας."}</p></div></div>${identityMarkup}
        ...
        <aside class="wpqs-workspace-side"><section class="panel"><h2>Ενεργές λειτουργίες</h2><div class="wpqs-feature-list">${
          Object.entries(features)
            .map(
              ([key, value]) =>
                `<span class="${bool(value) ? "is-enabled" : "is-disabled"}"><i>${bool(value) ? "✓" : "–"}</i>${esc(key.replaceAll("_", " "))}</span>`,
            )
            .join("") || '<p class="empty">Δεν έχουν οριστεί περιορισμοί.</p>'
        }</div>${!isSuper ? '<p class="wpqs-panel-note">Οι λειτουργίες ενεργοποιούνται ή απενεργοποιούνται από WordPress Administrator.</p>' : ""}</section><section class="panel"><h2>Χρήση θέσεων</h2><div class="wpqs-seat-numbers"><strong>${seatsUsed}</strong><span>/ ${number(org.seat_limit)}</span></div><div class="wpqs-seat-progress"><i style="width:${seatPercent}%"></i></div><p>${number(org.used_seats)} ενεργές · ${number(org.reserved_seats)} δεσμευμένες · ${seatsAvailable} διαθέσιμες</p><button data-workspace-team>Διαχείριση ομάδας</button></section></aside></div></main>`;
    bindPortalNav();
    root
      .querySelector("[data-workspace-team]")
      ?.addEventListener("click", () => routePortal("team"));
    root
      .querySelector("[data-open-organizations]")
      ?.addEventListener("click", () => routePortal("organizations"));
    root
      .querySelector("[data-workspace-save]")
      ?.addEventListener("click", async () => {
        const payload = { branding: {} };
        if (isSuper) {
          root
            .querySelectorAll("[data-workspace-field]")
            .forEach(
              (input) =>
                (payload[input.dataset.workspaceField] =
                  input.type === "number" ? number(input.value) : input.value),
            );
        }
        root
          .querySelectorAll("[data-workspace-branding]")
          .forEach(
            (input) =>
              (payload.branding[input.dataset.workspaceBranding] = input.value),
          );
        payload.embed_domains = root
          .querySelector("[data-workspace-embed-domains]")
          .value.split(/[\r\n,;]+/)
        ...
        if (isSuper) {
          ...
          payload.domains = [
            ...domains.filter(
              (item) => !["embed", "custom"].includes(item.domain_type),
            ),
            ...payload.embed_domains.map((domain) => ({
              domain,
              domain_type: "embed",
              is_primary: false,
            })),
        ...
        try {
          ...
          toast(
            isSuper
              ? "Το Workspace αποθηκεύτηκε"
              : "Τα domains και το branding αποθηκεύτηκαν",
          );
          renderWorkspace();
        } catch (error) {
          alert(error.message);
  ...
  const renderUserWorkspaces = async () => {
    ...
    if (!state.userWorkspaces)
      state.userWorkspaces = await api("admin/user-workspaces");
    const data = state.userWorkspaces || { users: [], organizations: [] };
    const query = String(state.userWorkspaceQuery || "").toLocaleLowerCase(
      "el",
    );
    const users = (data.users || []).filter(
      (user) =>
        !query ||
        `${user.display_name} ${user.user_email} ${(user.wordpress_roles || []).join(" ")} ${user.organization_name || ""}`
    ...
    bindPortalNav();
    root.querySelector("[data-user-workspace-search]").oninput = (event) => {
      state.userWorkspaceQuery = event.target.value;
      renderUserWorkspaces();
      root.querySelector("[data-user-workspace-search]")?.focus();
    };
    root.querySelectorAll("[data-user-workspace-save]").forEach(
      (button) =>
        (button.onclick = async () => {
          const id = button.dataset.userWorkspaceSave;
          const payload = {
            organization_id: number(
              root.querySelector(`[data-user-workspace-org="${id}"]`).value,
            ),
            org_role: root.querySelector(`[data-user-workspace-role="${id}"]`)
              .value,
            status: root.querySelector(`[data-user-workspace-status="${id}"]`)
              .value,
            move_quizzes: root.querySelector(
              `[data-user-workspace-move-quizzes="${id}"]`,
            ).checked,
          };
          if (
            !confirm(
              payload.move_quizzes
                ? "Να αλλάξει Workspace και να μεταφερθούν και τα quiz του χρήστη;"
                : "Να αλλάξει το Workspace του χρήστη; Τα παλιά quiz θα παραμείνουν στο προηγούμενο Workspace.",
            )
          )
            return;
          try {
            await api(`admin/users/${id}/workspace`, {
              method: "PUT",
              body: JSON.stringify(payload),
            });
            state.userWorkspaces = await api("admin/user-workspaces");
            state.team = null;
            state.workspace = null;
            toast("Το Workspace του χρήστη ενημερώθηκε");
            renderUserWorkspaces();
          } catch (error) {
            alert(error.message);
          }
  ...
  const defaultOrganization = () => ({
    name: "Νέος οργανισμός",
    slug: "",
    seat_limit: 10,
    creator_admin_limit: 1,
    status: "active",
    expires_at: null,
    domains: [{ domain: "", domain_type: "both", is_primary: true }],
    features: {
      analytics: true,
      templates: true,
      personality: true,
  ...
  const renderOrganizations = async () => {
    ...
    bindPortalNav();
    root.querySelector("[data-org-new]")?.addEventListener("click", () => {
      state.organizationEditor = defaultOrganization();
      renderOrganizations();
    });
    root.querySelectorAll("[data-org-edit]").forEach(
      (button) =>
        (button.onclick = async () => {
          state.organizationEditor = await api(
            `organizations/${button.dataset.orgEdit}`,
          );
          renderOrganizations();
        }),
    );
    root.querySelector("[data-org-cancel]")?.addEventListener("click", () => {
      state.organizationEditor = null;
      renderOrganizations();
    });
    root
      .querySelector("[data-org-save]")
      ?.addEventListener("click", async () => {
        const org = clone(state.organizationEditor);
        root.querySelectorAll("[data-org-field]").forEach((input) => {
          org[input.dataset.orgField] =
            input.type === "number" ? number(input.value) : input.value;
        });
        org.domains = root
          .querySelector("[data-org-domains]")
          .value.split(/[\r\n,;]+/)
          .filter(Boolean)
          .map((domain, index) => ({
            domain,
        ...
        org.features = {};
        root
          .querySelectorAll("[data-org-feature]")
          .forEach(
            (input) => (org.features[input.dataset.orgFeature] = input.checked),
          );
        org.branding = org.branding || {};
        root
          .querySelectorAll("[data-org-branding]")
          .forEach(
            (input) => (org.branding[input.dataset.orgBranding] = input.value),
          );
        await api(`organizations${org.id ? "/" + org.id : ""}`, {
  ...
  const documentationArticles = [
    {
      role: "creator",
      title: "Δημιουργία και αποθήκευση quiz",
      keywords: "creator quiz autosave questions",
      body: "Ανοίξτε τη Βιβλιοθήκη, πατήστε «Νέο Quiz», προσθέστε ερωτήσεις και επιλέξτε ορατότητα Private ή Organization. Η αυτόματη αποθήκευση εκτελείται ανά λίγα δευτερόλεπτα και οι χειροκίνητες αποθηκεύσεις δημιουργούν revisions.",
    },
    {
      role: "creator",
      title: "Υποβολή για έγκριση",
      keywords: "review workflow creator",
      body: "Από την καρτέλα Workflow γράψτε προαιρετικό σχόλιο και πατήστε «Υποβολή για έλεγχο». Μετά την υποβολή, ο Creator Admin μπορεί να εγκρίνει ή να ζητήσει αλλαγές.",
    },
    {
      role: "creator_admin",
      title: "Ομάδα, προσκλήσεις και θέσεις",
      keywords: "team seats invitations csv admin",
      body: "Στη σελίδα «Η ομάδα μου» βλέπετε ενεργούς χρήστες, διαθέσιμες θέσεις και εκκρεμείς προσκλήσεις. Μπορείτε να στείλετε μία ή πολλές προσκλήσεις ή να εισαγάγετε emails από CSV.",
    },
    {
      role: "creator_admin",
      title: "Έγκριση και δημοσίευση",
      keywords: "approval approve changes publish",
      body: "Στην καρτέλα Workflow ενός quiz μπορείτε να αφήσετε σχόλιο, να ζητήσετε αλλαγές, να εγκρίνετε ή να δημοσιεύσετε. Κάθε ενέργεια καταγράφεται στο Activity Log και αποστέλλεται email.",
    },
    {
      role: "administrator",
      title: "Organizations και domains",
      keywords: "organizations domains seats super admin",
      body: "Ο WordPress Administrator δημιουργεί Organizations, ορίζει domains, θέσεις, όριο Creator Admins, λήξη πρόσβασης, features, κατάσταση και white-label branding.",
    },
    {
      role: "administrator",
      title: "Ορατότητα Private / Organization / Universal",
      keywords: "visibility private organization universal",
      body: "Private βλέπει ο δημιουργός και οι admins του Organization. Organization βλέπουν όλοι οι εγκεκριμένοι χρήστες του ίδιου workspace. Universal βλέπουν όλοι οι εγκεκριμένοι χρήστες, αλλά το αρχικό περιεχόμενο το διαχειρίζεται μόνο ο Super Admin ή Universal Manager.",
    },
    {
      role: "all",
      title: "Ενσωμάτωση σε WordPress, Drupal ή άλλο CMS",
      keywords: "embed iframe javascript drupal wordpress gutenberg",
      body: "Ανοίξτε «Ενσωμάτωση» από την κάρτα του quiz. Για Gutenberg χρησιμοποιήστε block Shortcode ή Custom HTML. Για Drupal/Joomla/Laravel χρησιμοποιήστε το iframe ή το JavaScript embed. Η whitelist domains ελέγχει πού επιτρέπεται η φόρτωση.",
    },
    {
      role: "all",
      title: "Analytics και εξαγωγές",
      keywords: "analytics csv pdf reports",
      body: "Τα Analytics περιλαμβάνουν views, starts, completions, completion rate, score, χρόνο, funnel, drop-off ανά ερώτηση, συσκευές, referrers και UTM. Η εξαγωγή CSV είναι άμεση και το κουμπί Εκτύπωση / PDF ανοίγει print-friendly report.",
    },
    {
      role: "all",
      title: "Προσβασιμότητα",
      keywords: "accessibility keyboard wcag",
      body: "Χρησιμοποιήστε Tab και Shift+Tab για πλοήγηση, Enter ή Space για ενεργοποίηση controls και Escape για κλείσιμο popup. Τα presets ελέγχουν αντίθεση και υποστηρίζεται reduced motion.",
    },
  ];
  ...
  const renderSystemStatus = async () => {
    ...
    if (!state.systemHealth) {
      ...
      bindPortalNav();
      try {
        state.systemHealth = await api("system/health");
      } catch (error) {
        root.querySelector(".empty-panel").textContent = error.message;
        return;
      }
    }
    ...
    root.innerHTML = `<main class="wpqs-dashboard wpqs-portal-page wpqs-system-page">${portalNav("system")}
      ...
      <aside class="panel wpqs-environment-card"><h2>Περιβάλλον</h2><dl>${Object.entries(
        ...
        .map(
          ([key, value]) =>
            `<dt>${esc(key.replaceAll("_", " "))}</dt><dd>${esc(String(value))}</dd>`,
        )
        .join(
          "",
        )}</dl><p>Τελευταίος έλεγχος: ${esc(formatDate(report.generated_at))}</p><div class="notice-inline">Η επιδιόρθωση επανεκτελεί τις migrations, επαναφέρει το WP-Cron event και ανανεώνει τα rewrite rules χωρίς να διαγράφει δεδομένα.</div></aside></section>
    </main>`;
    bindPortalNav();
    root.querySelector("[data-system-refresh]").onclick = async () => {
    ...
    root.querySelector("[data-system-repair]").onclick = async (event) => {
      ...
      button.disabled = true;
      button.textContent = "Επιδιόρθωση…";
      try {
        state.systemHealth = await api("system/repair", {
          method: "POST",
          body: "{}",
        });
        toast("Η εγκατάσταση ελέγχθηκε και επιδιορθώθηκε");
        renderSystemStatus();
      } catch (error) {
        button.disabled = false;
        button.textContent = "Επιδιόρθωση εγκατάστασης";
        toast(error.message);
      }
    };
  };
  ...
  const renderDocumentation = () => {
    ...
    root.innerHTML = `
      <main class="wpqs-dashboard wpqs-portal-page">

        ${portalNav("documentation")}

        <header class="wpqs-dashboard-hero">
          <div>
            <span class="wpqs-kicker">HELP CENTER</span>
            <h1>Τεκμηρίωση Quiz Atelier</h1>
            <p>
              Οδηγίες ανά ρόλο για δημιουργία, έγκριση,
              Organizations, embeds και analytics.
            </p>
          </div>
        ...
        <section class="wpqs-list wpqs-docs-layout">

          <div class="wpqs-docs-toolbar">

            <input
              type="search"
              data-doc-search
              value="${esc(state.docsQuery || "")}"
              placeholder="Αναζήτηση στην τεκμηρίωση"
              autocomplete="off"
            >

            <select data-doc-role>
              <option
                value="all"
                ${role === "all" ? "selected" : ""}
              >
          ...
          <div class="wpqs-doc-grid">
            ...
            ${documentationArticles
              ...
              <article
                class="wpqs-doc-card"
                data-doc-card
                data-doc-role="${esc(article.role)}"
              >
                <span class="wpqs-kicker">
                  ${esc(
                    article.role === "all"
                      ? "ΟΛΟΙ"
                      : article.role.replace("_", " ").toUpperCase(),
                  )}
                </span>
          ...
          <div class="notice-inline">
            ...
            <code>[wp_quiz_studio_builder]</code>
            για το Creator Portal,

            <code>[wp_quiz_studio id="25"]</code>
            για ένα quiz και

            <code>[wp_quiz_studio_directory]</code>
            για δημόσιο κατάλογο.
          </div>
    ...
    const roleSelect = root.querySelector("[data-doc-role]");
    ...
    /*
     * Normalize text for Greek search.
     * Allows searches with/without accents.
     */
    ...
    /*
     * Filter WITHOUT rebuilding the page.
     */
    const applyDocumentationFilters = () => {
      ...
      cards.forEach((card) => {
        const cardRole = card.dataset.docRole || "all";
      ...
      /*
       * Update numbering only for visible results.
       */
      visibleCards.forEach((card, index) => {
        const numberNode = card.querySelector("[data-doc-number]");

        if (numberNode) {
          numberNode.textContent = `${index + 1} / ${visibleCards.length}`;
        }
      });
       ...
       */
      state.docsQuery = searchInput?.value || "";

      state.docsRole = selectedRole;
    };
     ...
     *
     * IMPORTANT:
     * No renderDocumentation() here.
     */
    searchInput?.addEventListener("input", applyDocumentationFilters);

    /*
     * Role filter
     *
     * Again: no full render.
     */
    roleSelect?.addEventListener("change", applyDocumentationFilters);

    /*
     * Print
     */
    const printButton = root.querySelector("[data-doc-print]");
  ...
  const renderProfile = () => {
    ...
    root.querySelector("[data-profile-save]").onclick = async () => {
      ...
      root
        ...
        .forEach((input) => (data[input.dataset.profileField] = input.value));
      data.email_preferences = {};
      root
        .querySelectorAll("[data-email-pref]")
        .forEach(
          (input) =>
            (data.email_preferences[input.dataset.emailPref] = input.checked),
        );
      try {
        state.me = await api("profile", {
          method: "PUT",
  ...
  const quickUpdateQuiz = async (id, changes, control) => {
    ...
    try {
      const updated = normaliseQuiz(
        await api(`quizzes/${id}/quick-update`, {
          method: "PUT",
    ...
    root.querySelectorAll("[data-quiz-item]").forEach((item) => {
      const matches =
        ...
        (state.listStatus === "all" ||
          item.dataset.status === state.listStatus) &&
        (state.listType === "all" || item.dataset.type === state.listType) &&
        (state.listCategory === "all" ||
          item.dataset.category === state.listCategory) &&
        (state.listScope === "all" || item.dataset.scope === state.listScope) &&
        (state.listWorkflow === "all" ||
          item.dataset.workflow === state.listWorkflow) &&
        (state.listCreator === "all" ||
          item.dataset.creator === state.listCreator);
      item.hidden = !matches;
      if (matches) visible++;
    });
    const counter = root.querySelector("[data-visible-count]");
    if (counter)
      counter.textContent = `${visible} από ${state.quizzes.length} quiz`;
  ...
  const renderList = () => {
    state.view = "list";
    state.portalView = "library";
    const completions =
      state.dashboardAnalytics?.overview?.completions ??
      state.dashboardAnalytics?.completions ??
      state.quizzes.reduce((sum, quiz) => sum + number(quiz.completions), 0);
    const views =
      state.dashboardAnalytics?.overview?.views ??
      state.dashboardAnalytics?.views ??
      state.quizzes.reduce((sum, quiz) => sum + number(quiz.views), 0);
    const adminTransferActions = WPQS.isFront
      ? ""
      : '<button class="wpqs-ghost" data-import>Εισαγωγή JSON</button><input type="file" data-import-file accept="application/json,.json" hidden>';
    const categoryFilterOptions = state.categories
      .map(
        (category) =>
          `<option value="${category.id}" ${String(state.listCategory) === String(category.id) ? "selected" : ""}>${esc(category.name)}</option>`,
      )
      .join("");
    const statusOptions = (status) =>
      `<option value="draft" ${status === "draft" ? "selected" : ""}>Πρόχειρο</option><option value="published" ${status === "published" ? "selected" : ""}>Δημοσιευμένο</option><option value="scheduled" ${status === "scheduled" ? "selected" : ""}>Προγραμματισμένο</option><option value="private" ${status === "private" ? "selected" : ""}>Ιδιωτικό</option><option value="expired" ${status === "expired" ? "selected" : ""}>Έληξε</option>`;
    const typeOptions = (type) =>
      quizTypes
        .map(
          (value) =>
            `<option value="${value}" ${type === value ? "selected" : ""}>${esc(quizTypeLabel(value))}</option>`,
        )
        .join("");
    const visibilityOptions = (scope) =>
      `<option value="personal" ${scope === "personal" ? "selected" : ""}>Private</option><option value="organization" ${scope === "organization" ? "selected" : ""}>Organization</option>${WPQS.canManageUniversal ? `<option value="universal" ${scope === "universal" ? "selected" : ""}>Universal</option>` : scope === "universal" ? '<option value="universal" selected>Universal</option>' : ""}`;
    const workflowOptions = (quiz) => {
      ...
      if (!WPQS.canReview)
        values =
          number(quiz.author_id) === number(state.me?.id)
            ? ["draft", "submitted"]
            : [current];
      ...
      return [...new Set(values)]
        .map(
          (value) =>
            `<option value="${value}" ${current === value ? "selected" : ""}>${esc(workflowLabel(value))}</option>`,
        )
        .join("");
    };
    const sortedQuizzes = [...state.quizzes].sort((a, b) => {
      ...
      if (state.listSort === "views_desc")
        return number(b.views) - number(a.views);
      if (state.listSort === "completions_desc")
        return number(b.completions) - number(a.completions);
      if (state.listSort === "created_desc")
    ...
    const itemMarkup = (quiz) => {
      ...
      const categoryName =
        quiz.category?.name || quiz.settings.category || "Χωρίς κατηγορία";
      const search =
        `${quiz.title} ${categoryName} ${quizTypeLabel(quiz.quiz_type)} ${statusLabel(quiz.status)} ${visibilityLabel(quiz.visibility_scope)} ${workflowLabel(quiz.workflow_status)} ${quiz.author_name || ""}`.toLocaleLowerCase(
          "el",
        );
    ...
    root
      .querySelector("[data-user-style-open]")
      ?.addEventListener("click", openUserStyleModal);
    root
      .querySelector("[data-open-categories]")
      ?.addEventListener("click", renderCategoriesPage);
    root
      .querySelector("[data-global-analytics]")
      ?.addEventListener("click", openGlobalAnalytics);
    root
    ...
    if (importInput)
      importInput.onchange = async (event) => {
        ...
        try {
          ...
          toast("Το quiz εισήχθη ως πρόχειρο");
          openQuiz(imported);
        } catch (error) {
          alert(error.message || "Το αρχείο JSON δεν είναι έγκυρο.");
        } finally {
          event.target.value = "";
        }
      };
    root.querySelector("[data-list-search]").oninput = (event) => {
      state.listQuery = event.target.value.toLocaleLowerCase("el");
      applyListFilters();
    };
    root.querySelector("[data-list-status]").onchange = (event) => {
      state.listStatus = event.target.value;
      applyListFilters();
    };
    root.querySelector("[data-list-type]").onchange = (event) => {
      state.listType = event.target.value;
      applyListFilters();
    };
    root.querySelector("[data-list-category]").onchange = (event) => {
      state.listCategory = event.target.value;
      applyListFilters();
    };
    root.querySelector("[data-list-scope]").onchange = (event) => {
      state.listScope = event.target.value;
      applyListFilters();
    };
    root.querySelector("[data-list-workflow]").onchange = (event) => {
      state.listWorkflow = event.target.value;
      applyListFilters();
    };
    root
      .querySelector("[data-list-creator]")
      ?.addEventListener("change", (event) => {
        state.listCreator = event.target.value;
        applyListFilters();
      });
    root
      .querySelector("[data-list-sort]")
      ?.addEventListener("change", (event) => {
        state.listSort = event.target.value;
        localStorage.setItem("wpqs_list_sort", state.listSort);
        renderList();
      });
    root.querySelectorAll("[data-list-view]").forEach(
      (button) =>
        (button.onclick = () => {
          state.listView = button.dataset.listView;
          localStorage.setItem("wpqs_list_view", state.listView);
          renderList();
        }),
    );
    root
      ...
      .forEach(
        (button) =>
          (button.onclick = async () =>
            openQuiz(await api(`quizzes/${button.dataset.edit}`))),
      );
    root
      .querySelectorAll("[data-preview-list]")
      .forEach(
        (button) =>
          (button.onclick = () =>
            window.open(
              `${WPQS.site}wpqs-embed/${button.dataset.previewList}/`,
              "_blank",
              "noopener",
            )),
      );
    root.querySelectorAll("[data-analytics]").forEach(
      (button) =>
        (button.onclick = async () => {
          const quiz = await api(`quizzes/${button.dataset.analytics}`);
          openQuiz(quiz, "analytics");
        }),
    );
    root.querySelectorAll("[data-duplicate]").forEach(
      (button) =>
        (button.onclick = async () => {
          try {
            const duplicate = await api(
              `quizzes/${button.dataset.duplicate}/duplicate`,
              { method: "POST", body: "{}" },
            );
            toast("Το quiz αντιγράφηκε");
            openQuiz(duplicate);
          } catch (error) {
            alert(error.message);
          }
        ...
        (button.onclick = async () => {
          try {
            const payload = await api(
              `quizzes/${button.dataset.export}/export`,
            );
            const slug = String(
              payload?.quiz?.slug || `quiz-${button.dataset.export}`,
            ).replace(/[^a-z0-9-_]+/gi, "-");
    ...
    root.querySelectorAll("[data-embed]").forEach(
      (button) =>
        (button.onclick = () => {
          const quiz = state.quizzes.find(
            (item) => number(item.id) === number(button.dataset.embed),
          );
          if (quiz) openEmbedModal(quiz);
        }),
    );
    root
    ...
    root
      ...
      .forEach(
        (select) =>
          (select.onchange = (event) =>
            quickUpdateQuiz(
              select.dataset.quickType,
              { quiz_type: event.target.value },
              select,
            )),
      );
    root
      .querySelectorAll("[data-quick-scope]")
      .forEach(
        (select) =>
          (select.onchange = (event) =>
            quickUpdateQuiz(
              select.dataset.quickScope,
              { visibility_scope: event.target.value },
              select,
            )),
      );
    root
      .querySelectorAll("[data-quick-workflow]")
      .forEach(
        (select) =>
          (select.onchange = (event) =>
            quickUpdateQuiz(
              select.dataset.quickWorkflow,
    ...
    root.querySelectorAll("[data-delete]").forEach(
      (button) =>
        (button.onclick = async () => {
          if (
            !confirm(
              "Να διαγραφεί το quiz μαζί με τα αποτελέσματα, τα στατιστικά και τις εκδόσεις του;",
            )
          )
            return;
          try {
            await api(`quizzes/${button.dataset.delete}`, { method: "DELETE" });
            toast("Το quiz διαγράφηκε");
            load();
          } catch (error) {
            alert(error.message);
          }
  ...
  const openQuiz = (quiz, tab = "questions") => {
    ...
    state = {
      ...
      activeQuestionKey: String(
        restoredQuiz.questions?.[0]?.settings?.key || "",
      ),
      dirty:
        restoredQuiz !== quiz &&
        Boolean(localStorage.getItem(recoveryKey(restoredQuiz))),
      conflict: null,
      autosaveFailures: 0,
    };
    renderBuilder();
  ...
  const navButton = (tab, label) =>
    `<button class="nav ${state.tab === tab ? "active" : ""}" data-tab="${tab}">${label}</button>`;

  const builderActionBar =
    () => `<footer class="wpqs-builder-actionbar ${state.conflict ? "has-conflict" : ""}">
    <div class="wpqs-builder-action-status">${
      state.conflict
        ...
        : `<span class="${!state.online ? "is-offline" : state.dirty ? "is-dirty" : "is-saved"}">${!state.online ? "● Offline — οι αλλαγές κρατούνται τοπικά" : state.dirty ? "● Μη αποθηκευμένες αλλαγές" : "✓ Όλες οι αλλαγές αποθηκεύτηκαν"}</span><small>Αυτόματη αποθήκευση μετά από 4 δευτερόλεπτα · Ctrl/⌘ + S για άμεση αποθήκευση</small>`
    }</div>
  ...
  const renderBuilder = () => {
    ...
    if (!state.activeQuestionKey && quiz.questions.length)
      state.activeQuestionKey = String(quiz.questions[0].settings.key || "");
  ...
  const introCard =
    () => `<section class="intro-card wpqs-intro-editor"><div class="section-heading"><div><span class="wpqs-kicker">ΕΙΣΑΓΩΓΙΚΗ ΣΕΛΙΔΑ</span><h2>Πρώτη εικόνα του quiz</h2><p>Τα πεδία αυτά εμφανίζονται πριν ξεκινήσουν οι ερωτήσεις.</p></div></div>
  ...
  const conditionMarkup = (question, index) => {
    ...
    const previous = state.quiz.questions
      .slice(0, index)
      .filter((item) => item.type !== "open_text");
    let rules = Array.isArray(condition.rules) ? condition.rules : [];
    if (!rules.length && previous.length)
      rules = [
        {
          operator: "equals",
          question_key: previous[0].settings.key,
          answer_key: previous[0].answers?.[0]?.content?.key || "",
        },
      ];
    if (!previous.length) {
    ...
    const rows = rules
      .map((rule, ruleIndex) => {
        const source =
          previous.find((item) => item.settings.key === rule.question_key) ||
          previous[0];
  ...
  const questionCard = (question, index) => {
    ...
    const isActive = state.activeQuestionKey
      ? state.activeQuestionKey === questionKey
      : index === 0;
    const issueCount = state.validationIssues.filter(
      (issue) => issue.questionIndex === index,
    ).length;
    const multiCorrect =
      question.type === "multiple_answers" || question.type === "open_text";
    ...
    const personalityFields = (answer, answerIndex) =>
      personality && profiles.length
      ...
      : question.answers
          .map(
            (
              answer,
              answerIndex,
            ) => `<div class="answer-row ${orderedType ? "is-order-item" : ""}" data-answer-row="${answerIndex}" data-question-index="${index}">
        ...
        <button type="button" class="answer-delete" data-delete-answer="${index}" data-answer="${answerIndex}" aria-label="Διαγραφή απάντησης">×</button>
          ...
          .join("");
    const advanced =
      question.type === "multiple_choice" || question.type === "image_choice"
    ...
    const typePanelDescriptions = {
      multiple_choice:
        "Επιλέξτε ακριβώς μία σωστή απάντηση και ορίστε τους βαθμούς της.",
      multiple_answers:
        "Επιλέξτε περισσότερες από μία σωστές απαντήσεις και τον τρόπο βαθμολόγησης.",
      true_false: "Ορίστε αν η σωστή επιλογή είναι «Σωστό» ή «Λάθος».",
      image_choice:
        "Προσθέστε εικόνα σε κάθε απάντηση και επιλέξτε μία σωστή επιλογή.",
      poll: "Οι επιλογές καταγράφονται ως ψήφοι χωρίς βαθμολόγηση.",
      open_text:
        "Οι γραμμές απαντήσεων παρακάτω είναι οι αποδεκτές λεκτικές απαντήσεις.",
      numeric:
        "Ορίστε τον σωστό αριθμό, το επιτρεπόμενο περιθώριο απόκλισης και τους βαθμούς.",
      slider:
        "Ορίστε τα όρια της κλίμακας, το βήμα και το εύρος τιμών που θεωρείται σωστό.",
      rating:
        "Ρυθμίστε τον αριθμό επιπέδων, την εμφάνιση και το σωστό εύρος αξιολόγησης.",
      ordering:
        "Η σειρά των στοιχείων μέσα στον builder αποτελεί τη σωστή σειρά.",
      ranking:
        "Η σειρά των στοιχείων μέσα στον builder αποτελεί την αναμενόμενη κατάταξη.",
      matching:
        "Κάθε αριστερή τιμή πρέπει να αντιστοιχεί στη δεξιά τιμή της ίδιας γραμμής.",
    };
    const advancedPanel = advanced
      ? `<section class="wpqs-type-panel"><div class="wpqs-type-panel-head"><strong>Ρυθμίσεις τύπου: ${esc(typeLabel(question.type))}</strong><small>${esc(typePanelDescriptions[question.type] || "Προσαρμόστε τον τρόπο λειτουργίας και βαθμολόγησης της ερώτησης.")}</small></div>${advanced}</section>`
      : "";
        ...
        : matchingType
          ...
          : orderedType
            ? question.type === "ranking"
              ? "Στοιχεία κατάταξης"
              : "Στοιχεία σωστής σειράς"
            : question.type === "poll"
              ? "Επιλογές δημοσκόπησης"
              : question.type === "image_choice"
                ? "Απαντήσεις με εικόνα"
                : "Απαντήσεις";
    const answerSectionDescription =
      question.type === "open_text"
        ? "Προσθέστε όλες τις λεκτικές μορφές που πρέπει να θεωρούνται σωστές."
        : matchingType
          ? "Συμπληρώστε αριστερή και δεξιά τιμή σε κάθε γραμμή."
          : orderedType
            ? "Μετακινήστε τις γραμμές ώστε να βρίσκονται στη σωστή σειρά."
            : question.type === "poll"
              ? "Οι επιλογές καταγράφονται ως ψήφοι και δεν έχουν σωστή απάντηση."
              : question.type === "multiple_answers"
                ? "Μπορείτε να επιλέξετε περισσότερες από μία σωστές απαντήσεις."
                : question.type === "true_false"
                  ? "Επιλέξτε ποια από τις δύο σταθερές επιλογές είναι σωστή."
                  : "Επιλέξτε τη σωστή απάντηση και ορίστε τη βαθμολογία της.";
        ...
        <p class="wpqs-question-type-help">${
          {
            ...
            multiple_answers:
              "Ο επισκέπτης μπορεί να επιλέξει περισσότερες από μία απαντήσεις.",
            true_false:
              "Οι επιλογές «Σωστό» και «Λάθος» δημιουργούνται αυτόματα.",
            image_choice: "Κάθε απάντηση μπορεί να έχει τη δική της εικόνα.",
            poll: "Καταγράφει ψήφους χωρίς σωστή ή λάθος απάντηση.",
            open_text:
              "Προσθέστε μία ή περισσότερες αποδεκτές λεκτικές απαντήσεις.",
            ...
            matching:
              "Κάθε αριστερή τιμή αντιστοιχεί στη δεξιά τιμή της ίδιας γραμμής.",
          }[question.type] ||
          "Η αλλαγή τύπου κρατά τις μη αποθηκευμένες αλλαγές σας."
        }</p>
      </div>
  ...
  const refreshQuestionCard = (index, focusSelector = "") => {
    ...
    requestAnimationFrame(() => {
      ...
      if (nextCard)
        window.scrollBy({
          top: nextCard.getBoundingClientRect().top - topBefore,
          left: 0,
          behavior: "auto",
        });
  ...
  const categoryManagerMarkup = (standalone = false) => {
    ...
    const rows = categories
      .map(
        (
          category,
        ) => `<article class="wpqs-category-row" data-category-item="${category.id}">
    ...
    const iconOptions = Object.keys(categoryIcons)
      .map(
        (icon) =>
          `<option value="${icon}">${esc(categoryIcons[icon])} ${esc(categoryIconLabel(icon))}</option>`,
      )
      .join("");
  ...
  const renderCategoriesPage = () => {
    ...
    const assigned = state.categories.reduce(
      (sum, category) => sum + number(category.quiz_count),
      0,
    );
    const uncategorized = state.quizzes.filter(
      (quiz) => !number(quiz.category_id),
    ).length;
    ...
    bindPortalNav();
    root
      .querySelector("[data-categories-back]")
      ?.addEventListener("click", renderList);
    bindCategoryEvents();
  };

  const categoryOptions = () =>
    `<option value="0">— Χωρίς κατηγορία —</option>${state.categories.map((category) => `<option value="${category.id}" ${number(state.quiz.category_id) === number(category.id) ? "selected" : ""}>${esc(category.name)}</option>`).join("")}`;

  const settingsTab =
    () => `<section class="panel"><h2>Ρυθμίσεις quiz</h2><div class="settings-grid two">
  ...
  const themeTab = () => {
    const preset =
      state.quiz.theme.preset === "custom"
        ? {
            ...state.quiz.theme,
            label: "Προσαρμοσμένο",
            description: "Το προσωπικό χρωματικό style αυτού του quiz.",
          }
        : quizThemePresets[state.quiz.theme.preset] || quizThemePresets.atelier;
    const options = `${state.quiz.theme.preset === "custom" ? '<option value="custom" selected>Προσαρμοσμένο</option>' : ""}${Object.entries(
      quizThemePresets,
    )
      .map(
        ([key, value]) =>
          `<option value="${key}" ${state.quiz.theme.preset === key ? "selected" : ""}>${esc(value.label)}</option>`,
      )
      .join("")}`;
  ...
  const trendSvg = (rows) => {
    ...
    if (!data.length)
      return '<div class="empty-panel">Δεν υπάρχουν δεδομένα για το επιλεγμένο διάστημα.</div>';
    const width = 760,
      height = 220,
      padding = 24;
    const max = Math.max(
      ...
      data
        .map((row, index) => {
          ...
          const y =
            height -
            padding -
            (number(row[key]) / max) * (height - padding * 2);
  ...
  const distributionMarkup = (title, rows, note = "") =>
    `<article class="analytics-distribution"><h3>${esc(title)}</h3>${note ? `<p>${esc(note)}</p>` : ""}<div>${
      (rows || [])
        .slice(0, 8)
        .map(
          (row) =>
            `<div class="distribution-row"><span>${esc(row.label)}</span><div><i style="width:${Math.max(1, number(row.percent))}%"></i></div><b>${number(row.value)} <small>${number(row.percent)}%</small></b></div>`,
        )
        .join("") || '<div class="empty">Χωρίς δεδομένα</div>'
    }</div></article>`;
  ...
  const downloadAnalyticsCsv = (analytics, global = false) => {
    ...
    const rows = [
      ...
      ...(analytics?.timeseries || analytics?.daily || []).map((item) => [
        "Χρονοσειρά",
        item.day,
        item.views,
        item.starts,
    ...
    if (global) {
      rows.push(
        [],
        ["Quiz", "Τύπος", "Views", "Starts", "Completions", "Completion rate"],
      );
      (analytics?.quiz_breakdown || []).forEach((item) =>
        rows.push([
          item.title,
          quizTypeLabel(item.quiz_type),
          item.views,
          item.starts,
          item.completions,
      ...
      (analytics?.questions || []).forEach((item) =>
        rows.push([
          item.title,
          item.reached,
          `${item.reach_rate}%`,
          ...
          `${item.wrong_percent}%`,
          `${item.skipped_percent}%`,
          `${item.average_time}s`,
          `${item.dropoff} (${item.dropoff_percent}%)`,
        ]),
      );
    }
    const distributions = {
      Συσκευές: analytics?.audience?.devices,
      Browsers: analytics?.audience?.browsers,
      Λειτουργικά: analytics?.audience?.operating_systems,
      Referrers: analytics?.audience?.referrers,
      Χώρες: analytics?.audience?.countries,
      Πόλεις: analytics?.audience?.cities,
      "UTM Source": analytics?.audience?.utm_sources,
      "UTM Campaign": analytics?.audience?.utm_campaigns,
      Αποτελέσματα: analytics?.result_distribution,
      Score: analytics?.score_distribution,
      "Pass/Fail": analytics?.pass_distribution,
    };
  ...
  const bindAnalyticsEvents = (global = false) => {
    ...
    root
      .querySelector("[data-analytics-preset]")
      ?.addEventListener("change", (event) => {
        state.analyticsPreset = event.target.value;
        rerender();
      });
    root
      .querySelector("[data-analytics-from]")
      ?.addEventListener("change", (event) => {
        state.analyticsFrom = event.target.value;
      });
    root
      .querySelector("[data-analytics-to]")
      ?.addEventListener("change", (event) => {
        state.analyticsTo = event.target.value;
      });
    root
      .querySelector("[data-analytics-group]")
      ?.addEventListener("change", (event) => {
        state.analyticsGroup = event.target.value;
      });
    root
      .querySelector("[data-analytics-refresh]")
      ?.addEventListener("click", () =>
        global ? loadGlobalAnalytics() : loadAnalytics(),
      );
    root
      .querySelector("[data-analytics-export]")
      ?.addEventListener("click", () =>
        downloadAnalyticsCsv(
          global ? state.dashboardAnalytics : state.analytics,
          global,
        ),
      );
    root
      .querySelector("[data-analytics-print]")
  ...
  const analyticsTab = () => {
    if (!state.quiz.id)
      return '<div class="empty-panel">Αποθηκεύστε πρώτα το quiz για να δείτε στατιστικά.</div>';
    if (state.loadingPanel || !state.analytics)
      return '<div class="empty-panel">Φόρτωση στατιστικών…</div>';
  ...
  const workflowTab = () => {
    ...
    if (!quiz.id)
      return '<div class="empty-panel">Αποθηκεύστε πρώτα το quiz για να χρησιμοποιήσετε το workflow.</div>';
    const isOwner = number(quiz.author_id) === number(state.me?.id);
    const canReview = bool(WPQS.canReview);
    const history = Array.isArray(quiz.review_history)
      ? quiz.review_history
      : [];
    const actions = [];
    if (
      isOwner &&
      ["draft", "changes_requested"].includes(quiz.workflow_status)
    )
      actions.push(
        '<button class="wpqs-primary" data-workflow-action="submitted">Υποβολή για έλεγχο</button>',
      );
    if (canReview) {
      actions.push(
        '<button data-workflow-action="changes_requested">Ζητήστε αλλαγές</button>',
      );
      actions.push('<button data-workflow-action="approved">Έγκριση</button>');
      if (WPQS.canPublish)
        actions.push(
          '<button class="wpqs-primary" data-workflow-action="published">Έγκριση & Δημοσίευση</button>',
        );
    }
    const workflowSteps = ["draft", "submitted", "approved", "published"];
    const effectiveStatus =
      quiz.workflow_status === "changes_requested"
        ? "submitted"
        : quiz.workflow_status;
  ...
  const bindWorkflowEvents = () => {
    root.querySelectorAll("[data-workflow-action]").forEach(
      (button) =>
        (button.onclick = async () => {
          const action = button.dataset.workflowAction;
          const comment =
            root.querySelector("[data-workflow-comment]")?.value || "";
          if (action === "submitted" && !runBuilderValidation(true)) return;
          if (action === "changes_requested" && !comment.trim())
            return alert("Γράψτε ποιες αλλαγές ζητάτε.");
          button.disabled = true;
          try {
            state.quiz = normaliseQuiz(
              await api(`quizzes/${state.quiz.id}/workflow`, {
                method: "POST",
                body: JSON.stringify({ action, comment }),
              }),
            );
            state.quizzes = state.quizzes.map((item) =>
              number(item.id) === number(state.quiz.id)
                ? { ...item, ...state.quiz }
                : item,
            );
            toast(
  ...
  const historyTab = () => {
    if (!state.quiz.id)
      return '<div class="empty-panel">Αποθηκεύστε πρώτα το quiz για να χρησιμοποιήσετε το ιστορικό.</div>';
    if (state.loadingPanel || !state.revisions)
      return '<div class="empty-panel">Φόρτωση ιστορικού…</div>';
  ...
  const previewHasResponse = (value) =>
    Array.isArray(value)
      ? value.length > 0
  ...
  const previewNormaliseText = (value, settings = {}) => {
    ...
    if (settings.text_ignore_accents !== false)
      output = output.normalize("NFD").replace(/\p{Diacritic}/gu, "");
    if (settings.text_ignore_punctuation !== false)
  ...
  const previewEvaluationFor = (question, response) => {
    ...
    if (state.quiz.quiz_type === "personality" || type === "poll")
      return { gradable: false, correct: null, score: 0, max: 0 };
    let correct = false,
      score = 0,
      max = Math.max(0, number(settings.points, 1));

    if (["multiple_choice", "true_false", "image_choice"].includes(type)) {
      const answer = question.answers.find(
        (item) => String(item.content.key) === String(response),
      );
      ...
      const expected = correctAnswers
        .map((item) => String(item.content.key))
        .sort();
      max =
        correctAnswers.reduce(
          (total, item) => total + Math.max(0, number(item.score)),
          0,
        ) || Math.max(0, number(settings.points, 1));
      correct =
          ...
          (total, key) =>
            ...
            (expected.includes(key)
              ? Math.max(
                  ...
                  number(
                    correctAnswers.find(
                      (item) => String(item.content.key) === key,
                    )?.score,
                  ),
                ) || share
              : -share),
      ...
      correct = accepted.some(
        (item) =>
          previewNormaliseText(item.content.text, settings) === value &&
          value !== "",
      );
      max = Math.max(
        0,
        ...accepted.map((item) => number(item.score)),
        number(settings.points, 1),
      );
      score = correct ? max : 0;
    } else if (type === "numeric") {
      correct =
        response !== "" &&
        Math.abs(number(response) - number(settings.numeric_answer)) <=
          Math.max(0, number(settings.numeric_tolerance));
      score = correct ? max : 0;
    } else if (["slider", "rating"].includes(type)) {
      const value = number(response);
      correct =
        value >= number(settings.correct_min) &&
        value <= number(settings.correct_max);
      score = correct ? max : 0;
    } else if (["ordering", "ranking"].includes(type)) {
      const expected = question.answers.map((answer) =>
        String(answer.content.key),
      );
      ...
      if (settings.order_scoring === "partial" && expected.length) {
        score =
          (selected.reduce(
            (total, value, index) =>
              total + (value === expected[index] ? 1 : 0),
            0,
          ) /
            expected.length) *
          max;
      ...
      const matches = question.answers.reduce(
        (total, answer) =>
          total +
          (String(response?.[answer.content.key] || "") ===
          String(answer.content.match_text || "")
            ? 1
            : 0),
        0,
      );
      correct =
        question.answers.length > 0 && matches === question.answers.length;
      score =
        settings.matching_scoring === "partial" && question.answers.length
          ? (matches / question.answers.length) * max
          : correct
            ? max
            : 0;
    }
  ...
  const previewFeedbackFor = (question, response) => {
    ...
    if (!evaluation.gradable)
      return { ...evaluation, heading: "Η απάντησή σας καταχωρήθηκε." };
    return {
      ...evaluation,
      heading: evaluation.correct ? "Σωστά!" : "Λάθος απάντηση",
    };
  };
  ...
  const previewResult = () => {
    ...
    score = Number(score.toFixed(2));
    max = Number(max.toFixed(2));
    const range = (state.quiz.settings.results || []).find(
      (item) => score >= number(item.min) && score <= number(item.max),
    );
    return { correct, total, score, max, range };
  };

  const previewAnswerLabel = (question, key) =>
    question.answers.find(
      (answer) => String(answer.content.key) === String(key),
    )?.content?.text || "";

  const previewMarkup = () => {
    const quiz = state.quiz;
    const font =
      quiz.theme.font === "serif"
        ? "Georgia,serif"
        : quiz.theme.font === "rounded"
          ? "ui-rounded,system-ui,sans-serif"
          : "system-ui,sans-serif";
    const category = state.categories.find(
      (item) => number(item.id) === number(quiz.category_id),
    );
    ...
    const progress = quiz.questions.length
      ? Math.round(((index + 1) / quiz.questions.length) * 100)
      : 0;
    if (state.preview.feedback) {
      const correctAnswers = (() => {
        if (["ordering", "ranking"].includes(question.type))
          return question.answers
            .map((answer) => answer.content.text)
            .filter(Boolean);
        if (question.type === "matching")
          return question.answers
            .map(
              (answer) =>
                `${answer.content.text} → ${answer.content.match_text}`,
            )
            .filter(Boolean);
        if (question.type === "numeric")
          return [String(question.settings.numeric_answer)];
        if (["slider", "rating"].includes(question.type))
          return [
            `${question.settings.correct_min} – ${question.settings.correct_max}`,
          ];
        return question.answers
    ...
    let answers = "";
    if (
      ["multiple_choice", "true_false", "image_choice", "poll"].includes(
        question.type,
      )
    ) {
      ...
      const min = number(question.settings.slider_min),
        max = number(question.settings.slider_max, 100),
        value = response === undefined ? min : number(response);
      answers = `<div class="wpqs-preview-slider"><output>${value}</output><input type="range" data-preview-slider min="${min}" max="${max}" step="${Math.max(0.01, number(question.settings.slider_step, 1))}" value="${value}"></div><button type="button" data-preview-submit>Συνέχεια</button>`;
    } else if (question.type === "rating") {
      const max = Math.max(
        2,
        Math.min(20, number(question.settings.rating_max, 5)),
      );
      ...
      if (!Array.isArray(state.preview.orderSeeds[questionKey]))
        state.preview.orderSeeds[questionKey] = shuffled(
          question.answers.map((answer) => answer.content.key),
        );
      const order =
        Array.isArray(response) && response.length
          ? response
          : state.preview.orderSeeds[questionKey];
  ...
  const previewSubmit = (surface, response) => {
    ...
    if (required && !previewHasResponse(response)) {
      toast("Δώστε μία απάντηση για να συνεχίσετε.");
      return;
    }
    state.preview.responses[String(question.settings.key)] = response;
    if (state.quiz.settings.show_feedback !== false)
      state.preview.feedback = previewFeedbackFor(question, response);
    else previewAdvance();
    refreshPreview();
  };
  ...
  const bindPreviewEvents = () => {
    root.querySelectorAll("[data-preview-surface]").forEach((surface) => {
      surface
        .querySelector("[data-preview-start]")
        ?.addEventListener("click", () => {
          state.preview.started = true;
          state.preview.index = previewFindIndex(0);
          state.preview.complete = state.preview.index < 0;
          refreshPreview();
        });
      surface
        .querySelector("[data-preview-restart]")
        ?.addEventListener("click", () => {
          resetPreview();
          refreshPreview();
        });
      surface
        ...
        ?.addEventListener("click", previewAdvance);
      surface
        .querySelector("[data-preview-skip]")
        ?.addEventListener("click", () => previewSubmit(surface, ""));
      surface
        .querySelectorAll("[data-preview-choice]")
        .forEach((button) =>
          button.addEventListener("click", () =>
            previewSubmit(surface, button.dataset.previewChoice),
          ),
        );
      surface.querySelectorAll("[data-preview-rating]").forEach((button) =>
        button.addEventListener("click", () => {
          state.preview.responses[
            String(state.quiz.questions[state.preview.index].settings.key)
          ] = number(button.dataset.previewRating);
          refreshPreview();
        }),
      );
      surface
        .querySelector("[data-preview-slider]")
      ...
      surface.querySelectorAll("[data-preview-order]").forEach((button) =>
        button.addEventListener("click", () => {
          const question = state.quiz.questions[state.preview.index];
          const key = String(question.settings.key);
          const order =
            Array.isArray(state.preview.responses[key]) &&
            state.preview.responses[key].length
              ? [...state.preview.responses[key]]
              : question.answers.map((answer) => answer.content.key);
          const index = number(button.dataset.previewOrder);
          const target = index + number(button.dataset.direction);
          if (target < 0 || target >= order.length) return;
          [order[index], order[target]] = [order[target], order[index]];
          state.preview.responses[key] = order;
          refreshPreview();
        }),
      );
      surface
        ...
        ?.addEventListener("click", () => {
          ...
          if (question.type === "multiple_answers")
            response = [
              ...surface.querySelectorAll("[data-preview-multiple]:checked"),
            ].map((input) => input.value);
          else if (question.type === "open_text")
            response =
              surface.querySelector("[data-preview-open]")?.value || "";
          else if (question.type === "numeric")
            response =
              surface.querySelector("[data-preview-number]")?.value || "";
          else if (question.type === "slider")
            response = number(
              surface.querySelector("[data-preview-slider]")?.value,
            );
          else if (question.type === "rating")
            response =
              state.preview.responses[String(question.settings.key)] || "";
          else if (["ordering", "ranking"].includes(question.type))
            response =
              state.preview.responses[String(question.settings.key)] ||
              state.preview.orderSeeds[String(question.settings.key)] ||
              question.answers.map((answer) => answer.content.key);
          else if (question.type === "matching") {
            response = {};
            surface
              .querySelectorAll("[data-preview-match]")
              .forEach((select) => {
                response[select.dataset.previewMatch] = select.value;
              });
          }
          previewSubmit(surface, response);
        });
    });
    root.querySelectorAll("[data-preview-reset]").forEach(
      (button) =>
        (button.onclick = () => {
          resetPreview();
          refreshPreview();
  ...
  const openBuilderPreviewModal = () => {
    resetPreview();
    const modal = document.createElement("div");
    modal.className = "wpqs-modal-backdrop wpqs-builder-preview-modal";
    ...
    ensureButtonTypes(modal);
    bindPreviewEvents();
    const close = () => modal.remove();
    modal.querySelector("[data-preview-modal-close]").onclick = close;
    modal.addEventListener("click", (event) => {
      if (event.target === modal) close();
    });
  };
  ...
  const openValidationIssue = (issue) => {
    ...
    if (
      ...
      state.quiz.questions[issue.questionIndex]
    )
      state.activeQuestionKey = String(
        state.quiz.questions[issue.questionIndex].settings.key || "",
      );
    renderBuilder();
    requestAnimationFrame(() =>
      requestAnimationFrame(() => {
        const target = issue.selector
          ? root.querySelector(issue.selector)
          : null;
        (
          target || root.querySelector(".wpqs-validation-summary")
  ...
  const runBuilderValidation = (publishing = true) => {
    ...
    if (!state.validationIssues.length) {
      toast("Ο έλεγχος ολοκληρώθηκε χωρίς σφάλματα.");
      return true;
    }
    state.tab = "questions";
    ...
    if (
      ...
      state.quiz.questions[first.questionIndex]
    )
      state.activeQuestionKey = String(
        state.quiz.questions[first.questionIndex].settings.key || "",
      );
    renderBuilder();
    requestAnimationFrame(() =>
      root
        .querySelector(".wpqs-validation-summary")
        ?.scrollIntoView({ behavior: "smooth", block: "start" }),
    );
    toast(
      `Βρέθηκαν ${state.validationIssues.length} σημεία που χρειάζονται διόρθωση.`,
    );
    return false;
  };
  ...
  const bindValidationEvents = () => {
    root
      ...
      .forEach((button) => (button.onclick = () => runBuilderValidation(true)));
    root
      .querySelectorAll("[data-validation-jump]")
      .forEach(
        (button) =>
          (button.onclick = () =>
            openValidationIssue(
              state.validationIssues[number(button.dataset.validationJump)],
            )),
      );
  };
  ...
  const bindBuilder = () => {
    ...
    root
      ...
      ?.addEventListener("click", async () => {
        try {
          const latest =
            state.conflict?.latest || (await api(`quizzes/${quiz.id}`));
          removeRecovery(state.quiz);
          state.quiz = normaliseQuiz(latest);
          state.conflict = null;
          state.dirty = false;
          state.validationIssues = [];
          toast("Φορτώθηκε η νεότερη έκδοση από τον server");
          renderBuilder();
        } catch (error) {
          toast(error.message);
        }
    ...
    root
      .querySelector("[data-conflict-copy]")
      ?.addEventListener("click", async () => {
        ...
        copy.visibility_scope = "personal";
        copy.title = `${copy.title} (Αντίγραφο)`;
        ...
        try {
          ...
          state.quiz = saved;
          state.dirty = false;
          toast("Οι αλλαγές αποθηκεύτηκαν ως νέο αντίγραφο");
          renderBuilder();
        } catch (error) {
          toast(error.message);
        }
    ...
    root
      ...
      .forEach((button) => (button.onclick = () => save(null, false)));
    root
      .querySelectorAll("[data-publish]")
      .forEach((button) =>
        button.addEventListener("click", () => save("published", false)),
      );
    root
      .querySelectorAll("[data-preview]")
      .forEach((button) =>
        button.addEventListener("click", openBuilderPreviewModal),
      );
    root
      .querySelector("[data-builder-embed]")
      ?.addEventListener("click", () => openEmbedModal(quiz));
    root
      .querySelector("[data-save-template]")
      ?.addEventListener("click", async () => {
        const title = prompt("Τίτλος template", quiz.title);
        if (!title) return;
        const scope =
          WPQS.canManageUniversal &&
          confirm(
            "Πατήστε OK για Universal template ή Ακύρωση για template του Organization.",
          )
            ? "universal"
            : "organization";
        try {
          ...
          toast("Το template αποθηκεύτηκε");
        } catch (error) {
          alert(error.message);
        }
      });
    root.querySelector(".quiz-title").oninput = (event) => {
      quiz.title = event.target.value;
      const introTitleField = root.querySelector('[data-field="title"]');
      if (introTitleField && introTitleField !== event.target)
        introTitleField.value = event.target.value;
      markDirty();
      refreshPreview();
    };
    root
    ...
    root.querySelectorAll("[data-restore]").forEach(
      (button) =>
        (button.onclick = async () => {
          if (
            !confirm(
              "Να γίνει επαναφορά αυτής της έκδοσης; Η τρέχουσα έκδοση θα αποθηκευτεί πρώτα.",
            )
          )
            return;
          try {
            state.quiz = normaliseQuiz(
              await api(
                `quizzes/${quiz.id}/revisions/${button.dataset.restore}/restore`,
                { method: "POST", body: "{}" },
              ),
            );
            state.dirty = false;
            toast("Η έκδοση επαναφέρθηκε");
            renderBuilder();
          } catch (error) {
            alert(error.message);
          }
  ...
  const bindQuestionEvents = () => {
    const quiz = state.quiz;
    root.querySelectorAll("[data-question-toggle]").forEach(
      (button) =>
        (button.onclick = (event) => {
          event.preventDefault();
          const index = number(button.dataset.questionToggle);
          const question = quiz.questions[index];
          if (!question) return;
          state.activeQuestionKey =
            state.activeQuestionKey === String(question.settings.key)
              ? ""
              : String(question.settings.key);
          preserveBuilderPosition(
            renderBuilder,
            `[data-question-card="${index}"]`,
          );
        }),
    );
    root.querySelectorAll("[data-question-drag]").forEach((handle) => {
      handle.ondragstart = (event) => {
        state.dragQuestionIndex = number(handle.dataset.questionDrag);
        event.dataTransfer.effectAllowed = "move";
        event.dataTransfer.setData(
          "text/plain",
          String(state.dragQuestionIndex),
        );
        handle.closest(".question-card")?.classList.add("is-dragging");
      };
      handle.ondragend = () => {
    ...
    root.querySelectorAll("[data-question-card]").forEach((card) => {
      ...
      card.ondrop = (event) => {
        if (state.dragQuestionIndex === null) return;
        event.preventDefault();
        const target = number(card.dataset.questionCard);
        const source = state.dragQuestionIndex;
        card.classList.remove("is-drag-over");
        if (source === target) return;
        const [moved] = quiz.questions.splice(source, 1);
        quiz.questions.splice(target, 0, moved);
        state.activeQuestionKey = String(moved.settings.key);
        state.dragQuestionIndex = null;
        markDirty();
        renderBuilder();
      };
    ...
    root.querySelectorAll("[data-answer-drag]").forEach((handle) => {
      handle.ondragstart = (event) => {
        state.dragAnswer = {
          question: number(handle.dataset.question),
          answer: number(handle.dataset.answer),
        };
        event.dataTransfer.effectAllowed = "move";
        event.dataTransfer.setData(
          "text/plain",
          `${handle.dataset.question}:${handle.dataset.answer}`,
        );
        handle.closest(".answer-row")?.classList.add("is-dragging");
      };
      handle.ondragend = () => {
        state.dragAnswer = null;
        root
    ...
    root.querySelectorAll("[data-answer-row]").forEach((row) => {
      row.ondragover = (event) => {
        if (
          !state.dragAnswer ||
          state.dragAnswer.question !== number(row.dataset.questionIndex)
        )
          return;
        event.preventDefault();
      ...
      row.ondrop = (event) => {
        if (!state.dragAnswer) return;
        event.preventDefault();
        const questionIndex = number(row.dataset.questionIndex);
        if (state.dragAnswer.question !== questionIndex) return;
        const target = number(row.dataset.answerRow);
        const source = state.dragAnswer.answer;
        if (source === target) return;
        const answers = quiz.questions[questionIndex].answers;
        const [moved] = answers.splice(source, 1);
        answers.splice(target, 0, moved);
        state.dragAnswer = null;
        markDirty();
    ...
    const setField = (path, value) => {
      if (path === "title") {
        quiz.title = value;
        const topTitle = root.querySelector(".quiz-title");
        if (topTitle && topTitle.value !== value) topTitle.value = value;
      }
    ...
    root
      ...
      .forEach(
        (input) =>
          (input.oninput = (event) =>
            setField(input.dataset.field, event.target.value)),
      );
    root.querySelectorAll("[data-question-settings]").forEach(
      (details) =>
        (details.ontoggle = () => {
          const index = number(details.dataset.questionSettings);
          if (details.open) state.openQuestionSettings = index;
          else if (state.openQuestionSettings === index)
            state.openQuestionSettings = null;
        }),
    );
    root
      .querySelectorAll("[data-add-question]")
      .forEach((addQuestionButton) => {
        addQuestionButton.onclick = (event) => {
          ...
          state.activeQuestionKey = String(
            quiz.questions[index].settings.key || "",
          );
          markDirty();
          preserveBuilderPosition(
            renderBuilder,
    ...
    root.querySelectorAll("[data-question-title]").forEach(
      (input) =>
        (input.oninput = (event) => {
          const index = number(input.dataset.questionTitle);
          quiz.questions[index].content.title = event.target.value;
          const summary = input
            .closest(".question-card")
            ?.querySelector(".wpqs-question-summary strong");
          if (summary)
            summary.textContent = event.target.value.trim() || "Χωρίς τίτλο";
          markDirty();
          refreshPreview();
        }),
    );
    root.querySelectorAll("[data-question-type]").forEach(
      (select) =>
        (select.onchange = (event) => {
          event.preventDefault();
          event.stopPropagation();
          ...
          toast(
            `Ενεργοποιήθηκε ο τύπος «${typeLabel(nextType)}». Τα αντίστοιχα πεδία εμφανίστηκαν παρακάτω.`,
          );
        }),
    );
    root.querySelectorAll("[data-true-false-correct]").forEach(
      (select) =>
        (select.onchange = (event) => {
          event.preventDefault();
          event.stopPropagation();
          ...
          question.answers.forEach((answer, answerIndex) => {
            ...
            answer.score = answer.is_correct
              ? Math.max(1, number(question.settings.points, 1))
              : 0;
          });
          markDirty();
          refreshQuestionCard(
            index,
            `[data-true-false-correct][data-index="${index}"]`,
          );
        }),
    );
    root.querySelectorAll("[data-answer-question]").forEach(
      (input) =>
        (input.oninput = (event) => {
          quiz.questions[number(input.dataset.answerQuestion)].answers[
            number(input.dataset.answer)
          ].content.text = event.target.value;
          markDirty();
        }),
    );
    root.querySelectorAll("[data-answer-match]").forEach(
      (input) =>
        (input.oninput = (event) => {
          quiz.questions[number(input.dataset.question)].answers[
            number(input.dataset.answer)
          ].content.match_text = event.target.value;
          markDirty();
        }),
    );
    root.querySelectorAll("[data-personality-weight]").forEach(
      (input) =>
        (input.oninput = (event) => {
          const answer =
            quiz.questions[number(input.dataset.question)].answers[
          ...
          answer.content.personality_weights =
            answer.content.personality_weights || {};
          answer.content.personality_weights[input.dataset.profile] = number(
            event.target.value,
          );
          markDirty();
        }),
    );
    root.querySelectorAll("[data-score-question]").forEach(
      (input) =>
        (input.oninput = (event) => {
          quiz.questions[number(input.dataset.scoreQuestion)].answers[
            number(input.dataset.scoreAnswer)
          ].score = number(event.target.value);
          markDirty();
        }),
    );
    root.querySelectorAll("[data-correct-question]").forEach(
      (input) =>
        (input.onchange = () => {
          const question =
            quiz.questions[number(input.dataset.correctQuestion)];
          const index = number(input.dataset.correctAnswer);
          if (
            question.type === "multiple_answers" ||
            question.type === "open_text"
          ) {
            question.answers[index].is_correct = input.checked;
            if (input.checked && number(question.answers[index].score) <= 0)
              question.answers[index].score = 1;
          } else {
            question.answers.forEach((answer, answerIndex) => {
    ...
    root.querySelectorAll("[data-add-answer]").forEach(
      (button) =>
        (button.onclick = (event) => {
          event.preventDefault();
          event.stopPropagation();
          const questionIndex = number(button.dataset.addAnswer);
          const question = quiz.questions[questionIndex];
          question.answers.push(newAnswer("", false, 0));
          markDirty();
          const answerIndex = question.answers.length - 1;
          preserveBuilderPosition(
            renderBuilder,
            `[data-answer-question="${questionIndex}"][data-answer="${answerIndex}"]`,
          );
        }),
    );
    root.querySelectorAll("[data-delete-answer]").forEach(
      (button) =>
        (button.onclick = (event) => {
          event.preventDefault();
          const questionIndex = number(button.dataset.deleteAnswer);
          const question = quiz.questions[questionIndex];
          if (question.answers.length <= 1) {
            toast("Η ερώτηση πρέπει να έχει τουλάχιστον μία απάντηση.");
            return;
          }
          question.answers.splice(number(button.dataset.answer), 1);
          normaliseQuestionCorrectness(question);
          markDirty();
          preserveBuilderPosition(renderBuilder);
        }),
    );
    root.querySelectorAll("[data-move-answer]").forEach(
      (button) =>
        (button.onclick = () => {
          const answers =
            quiz.questions[number(button.dataset.moveAnswer)].answers;
    ...
    root.querySelectorAll("[data-delete-question]").forEach(
      (button) =>
        (button.onclick = () => {
          if (confirm("Να διαγραφεί αυτή η ερώτηση;")) {
            ...
            state.openQuestionSettings = null;
            state.activeQuestionKey = String(
              quiz.questions[Math.min(index, quiz.questions.length - 1)]
                ?.settings?.key || "",
            );
            markDirty();
            renderBuilder();
    ...
    root.querySelectorAll("[data-duplicate-question]").forEach(
      (button) =>
        (button.onclick = () => {
          const index = number(button.dataset.duplicateQuestion);
          const copy = cloneQuestionTemplate(quiz.questions[index]);
          quiz.questions.splice(index + 1, 0, copy);
          state.activeQuestionKey = String(copy.settings.key || "");
          markDirty();
          renderBuilder();
        }),
    );
    root.querySelectorAll("[data-save-bank]").forEach(
      (button) =>
        (button.onclick = async () => {
          const question = quiz.questions[number(button.dataset.saveBank)];
          const title = window.prompt(
            "Τίτλος στη βιβλιοθήκη ερωτήσεων:",
            question.content.title || "Επαναχρησιμοποιήσιμη ερώτηση",
          );
          if (title === null) return;
          try {
            state.questionBank = await api("question-bank", {
              method: "POST",
    ...
    root.querySelectorAll("[data-move-question]").forEach(
      (button) =>
        (button.onclick = () => {
          const index = number(button.dataset.moveQuestion);
          const target = index + number(button.dataset.direction);
          if (target < 0 || target >= quiz.questions.length) return;
          [quiz.questions[index], quiz.questions[target]] = [
    ...
    root.querySelectorAll("[data-condition-enabled]").forEach(
      (input) =>
        (input.onchange = () => {
          ...
          if (
            !Array.isArray(question.settings.condition.rules) ||
            !question.settings.condition.rules.length
          ) {
    ...
    root.querySelectorAll("[data-condition-match]").forEach(
      (select) =>
        (select.onchange = () => {
          quiz.questions[
            number(select.dataset.index)
          ].settings.condition.match = select.value;
          markDirty();
        }),
    );
    root.querySelectorAll("[data-condition-rule-question]").forEach(
      (select) =>
        (select.onchange = () => {
          const index = number(select.dataset.index);
          state.openQuestionSettings = index;
          const question = quiz.questions[index];
          const rule =
            question.settings.condition.rules[number(select.dataset.rule)];
          const source = quiz.questions.find(
            (item) => item.settings.key === select.value,
          );
    ...
    root.querySelectorAll("[data-condition-rule-operator]").forEach(
      (select) =>
        (select.onchange = () => {
          const index = number(select.dataset.index);
          state.openQuestionSettings = index;
          const question = quiz.questions[index];
          question.settings.condition.rules[
            number(select.dataset.rule)
          ].operator = select.value;
          markDirty();
          renderBuilder();
        }),
    );
    root.querySelectorAll("[data-condition-rule-answer]").forEach(
      (select) =>
        (select.onchange = () => {
          quiz.questions[number(select.dataset.index)].settings.condition.rules[
            number(select.dataset.rule)
          ].answer_key = select.value;
          markDirty();
        }),
    );
    root.querySelectorAll("[data-condition-add-rule]").forEach(
      (button) =>
        (button.onclick = () => {
          ...
          if (!source) return;
          question.settings.condition.rules = Array.isArray(
            question.settings.condition.rules,
          )
            ? question.settings.condition.rules
            : [];
          question.settings.condition.rules.push({
            operator: "equals",
            question_key: source.settings.key,
            answer_key: source.answers?.[0]?.content?.key || "",
          });
          markDirty();
          renderBuilder();
        }),
    );
    root.querySelectorAll("[data-condition-delete-rule]").forEach(
      (button) =>
        (button.onclick = () => {
          const index = number(button.dataset.index);
          state.openQuestionSettings = index;
    ...
    root.querySelectorAll("[data-question-setting]").forEach((input) => {
      const handler = (event) => {
        ...
        if (key === "points" && singleCorrectTypes.includes(question.type)) {
          question.answers.forEach((answer) => {
            answer.score = bool(answer.is_correct)
              ? Math.max(0, number(question.settings.points, 1))
              : 0;
          });
        }
        markDirty();
  ...
  const bindBankEvents = () => {
    root.querySelectorAll("[data-bank-insert]").forEach(
      (button) =>
        (button.onclick = () => {
          const item = state.questionBank.find(
            (entry) => number(entry.id) === number(button.dataset.bankInsert),
          );
    ...
    root.querySelectorAll("[data-bank-delete]").forEach(
      (button) =>
        (button.onclick = async () => {
          if (!confirm("Να διαγραφεί αυτή η ερώτηση από την τράπεζα;")) return;
          try {
            await api(`question-bank/${button.dataset.bankDelete}`, {
              method: "DELETE",
            });
            state.questionBank = state.questionBank.filter(
              (item) => number(item.id) !== number(button.dataset.bankDelete),
            );
  ...
  const bindCategoryEvents = () => {
    ...
    const preview = () => {
      ...
      element.querySelector("span").textContent =
        categoryIcons[icon] || categoryIcons.folder;
      element.querySelector("b").textContent =
        nameInput?.value.trim() || "Νέα κατηγορία";
      element.querySelector("small").textContent =
        slugInput?.value.trim() || "category-slug";
    };
    const clearForm = () => {
      editingId = 0;
    ...
    const openForm = (category) => {
      ...
      if (kicker)
        kicker.textContent = editingId ? "ΕΠΕΞΕΡΓΑΣΙΑ" : "ΝΕΑ ΚΑΤΗΓΟΡΙΑ";
      preview();
      nameInput?.focus();
      form?.scrollIntoView({ behavior: "smooth", block: "nearest" });
    };
    const rerender = () =>
      state.view === "categories" ? renderCategoriesPage() : renderBuilder();

    root
      .querySelector("[data-category-new]")
      ?.addEventListener("click", () => openForm(null));
    root
      .querySelector("[data-category-cancel]")
      ?.addEventListener("click", clearForm);
    root
      .querySelectorAll("[data-category-edit]")
      .forEach(
        (button) =>
          (button.onclick = () =>
            openForm(
              state.categories.find(
                (category) =>
                  number(category.id) === number(button.dataset.categoryEdit),
              ),
            )),
      );
    root
      .querySelector("[data-category-search]")
      ?.addEventListener("input", (event) => {
        state.categoryQuery = event.target.value;
        const query = state.categoryQuery.trim().toLocaleLowerCase("el");
        root.querySelectorAll("[data-category-item]").forEach((item) => {
          const category = state.categories.find(
            (entry) => number(entry.id) === number(item.dataset.categoryItem),
          );
          const haystack =
            `${category?.name || ""} ${category?.slug || ""} ${category?.description || ""}`.toLocaleLowerCase(
              "el",
            );
          item.hidden = Boolean(query && !haystack.includes(query));
        });
      });
    [nameInput, slugInput, descriptionInput].forEach((input) =>
    ...
    colorTextInput?.addEventListener("change", () => {
      if (/^#[0-9a-f]{6}$/i.test(colorTextInput.value) && colorInput)
        colorInput.value = colorTextInput.value;
      preview();
    });
    iconInput?.addEventListener("change", preview);
    root
      .querySelector("[data-category-save]")
      ?.addEventListener("click", async () => {
        const name = nameInput?.value.trim() || "";
        if (!name) {
          alert("Συμπληρώστε όνομα κατηγορίας.");
          nameInput?.focus();
          return;
        }
        ...
        try {
          const saved = await api(
            `categories${editingId ? "/" + editingId : ""}`,
            {
              ...
              body: JSON.stringify({
                ...
                color: colorInput?.value || "#d9bd85",
                icon: iconInput?.value || "folder",
              }),
            },
          );
          const index = state.categories.findIndex(
            (category) => number(category.id) === number(saved.id),
          );
          ...
          toast(
            editingId ? "Η κατηγορία ενημερώθηκε" : "Η κατηγορία δημιουργήθηκε",
          );
          rerender();
        } catch (error) {
          button.disabled = false;
    ...
    root.querySelectorAll("[data-category-delete]").forEach(
      (button) =>
        (button.onclick = async () => {
          const id = number(button.dataset.categoryDelete);
          if (
            !confirm(
              "Να διαγραφεί η κατηγορία; Τα quiz θα παραμείνουν χωρίς κατηγορία.",
            )
          )
            return;
          try {
            ...
            if (state.quiz && number(state.quiz.category_id) === id) {
              state.quiz.category_id = 0;
              state.quiz.category = null;
              markDirty();
            }
    ...
    root.querySelectorAll("[data-setting]").forEach((input) => {
      ...
      input.addEventListener(eventName, (event) => {
        ...
        if (key === "visibility_scope")
          quiz.visibility_scope = event.target.value;
        if (key === "category_id") {
          quiz.category_id = number(event.target.value);
          const category = state.categories.find(
            (item) => number(item.id) === quiz.category_id,
          );
        ...
        if (key === "show_progress")
          quiz.settings.show_progress = input.checked;
        if (key === "random_questions")
          quiz.settings.random_questions = input.checked;
        if (key === "random_question_limit")
          quiz.settings.random_question_limit = number(event.target.value);
        if (key === "review_answers")
          quiz.settings.review_answers = input.checked;
        if (key === "show_pass_fail")
          quiz.settings.show_pass_fail = input.checked;
        if (key === "pass_score")
          quiz.settings.pass_score = number(event.target.value);
        if (key === "embed_mode") quiz.settings.embed_mode = event.target.value;
        if (key === "embed_domains")
          quiz.settings.embed_domains = event.target.value
            .split(/[\r\n,;]+/)
            .map((value) => value.trim())
            .filter(Boolean);
        if (key === "embed_block_message")
          quiz.settings.embed_block_message = event.target.value;
        if (key === "show_feedback")
          quiz.settings.show_feedback = input.checked;
        if (key === "show_correct_answer")
          quiz.settings.show_correct_answer = input.checked;
        if (key === "allow_restart")
          quiz.settings.allow_restart = input.checked;
        if (key === "status") {
        ...
        if (key === "scheduled_at")
          quiz.scheduled_at = event.target.value
            ? new Date(event.target.value).toISOString()
            : null;
        if (key === "expires_at")
          quiz.expires_at = event.target.value
            ? new Date(event.target.value).toISOString()
            : null;
        markDirty();
      });
  ...
  const updateContrastReport = () => {
    ...
    const checks = [
      ...
      [
        "Δευτερεύον / κάρτα",
        state.quiz.theme.muted,
        state.quiz.theme.background,
      ],
      [
        "Κείμενο / κουμπί",
        state.quiz.theme.button_text,
        state.quiz.theme.button,
      ],
  ...
  const bindThemeEvents = () => {
    ...
    root.querySelectorAll("[data-theme]").forEach((input) => {
      const handler = (event) => {
        ...
        quiz.theme[key] =
          key === "radius" ? number(event.target.value) : event.target.value;
        if (key !== "font" && key !== "shadow" && key !== "radius")
          quiz.theme.preset = "custom";
        const sibling = input.parentElement?.querySelector("span");
        if (key === "radius" && sibling)
          sibling.textContent = `${quiz.theme.radius}px`;
    ...
    root.querySelectorAll("[data-theme-text]").forEach(
      (input) =>
        (input.onchange = (event) => {
    ...
    root
      .querySelector("[data-theme-preset-select]")
      ?.addEventListener("change", (event) => {
        const preset =
          quizThemePresets[event.target.value] || quizThemePresets.atelier;
    ...
    root
      .querySelector("[data-theme-auto-contrast]")
      ?.addEventListener("click", () => {
        quiz.theme.text = bestTextColor(quiz.theme.background);
        quiz.theme.button_text = bestTextColor(quiz.theme.button);
        quiz.theme.muted =
          quiz.theme.text === "#ffffff" ? "#d4d4d8" : "#374151";
        if (contrastRatio(quiz.theme.text, quiz.theme.answer) < 4.5)
          quiz.theme.answer = quiz.theme.background;
        quiz.theme.preset = "custom";
        markDirty();
        renderBuilder();
      });
  ...
  const bindResultEvents = () => {
    ...
    root.querySelectorAll("[data-result]").forEach(
      (input) =>
        (input.oninput = (event) => {
          const range = ranges[number(input.dataset.index)];
          range[input.dataset.result] =
            input.type === "number"
              ? number(event.target.value)
              : event.target.value;
          markDirty();
    ...
    root.querySelectorAll("[data-delete-result]").forEach(
      (button) =>
        (button.onclick = () => {
          ranges.splice(number(button.dataset.deleteResult), 1);
          markDirty();
          renderBuilder();
        }),
    );
    root.querySelector("[data-add-profile]")?.addEventListener("click", () => {
      profiles.push({
        key: makeKey("profile"),
        title: `Προφίλ ${profiles.length + 1}`,
        description: "",
        image_id: 0,
        image_url: "",
        cta_label: "",
    ...
    root.querySelectorAll("[data-profile]").forEach(
      (input) =>
        (input.oninput = (event) => {
          const profile = profiles[number(input.dataset.index)];
          const oldKey = profile.key;
          profile[input.dataset.profile] = event.target.value;
          if (input.dataset.profile === "key") {
            ...
            state.quiz.questions.forEach((question) =>
              question.answers.forEach((answer) => {
                const weights = answer.content.personality_weights || {};
                if (
                  oldKey &&
                  oldKey !== clean &&
                  Object.prototype.hasOwnProperty.call(weights, oldKey)
                ) {
                  weights[clean] = weights[oldKey];
                  delete weights[oldKey];
                }
              }),
            );
          }
    ...
    root.querySelectorAll("[data-delete-profile]").forEach(
      (button) =>
        (button.onclick = () => {
          const index = number(button.dataset.deleteProfile);
          const key = profiles[index]?.key;
          profiles.splice(index, 1);
          if (key)
            state.quiz.questions.forEach((question) =>
              question.answers.forEach((answer) => {
                if (answer.content.personality_weights)
                  delete answer.content.personality_weights[key];
              }),
            );
          markDirty();
          renderBuilder();
    ...
    root.querySelectorAll("[data-personality-setting]").forEach(
      (select) =>
        (select.onchange = () => {
          state.quiz.settings[select.dataset.personalitySetting] = select.value;
          markDirty();
        }),
    );
  };
  ...
  const bindMediaEvents = () => {
      ...
      (button) =>
        (button.onclick = () =>
          chooseMedia((attachment) => {
            const target = button.dataset.media;
            const index = number(button.dataset.index);
            if (target === "intro") {
              state.quiz.settings.intro.image_id = attachment.id;
              state.quiz.settings.intro.image_url = attachment.url;
            }
            if (target === "question") {
              state.quiz.questions[index].content.image_id = attachment.id;
              state.quiz.questions[index].content.image_url = attachment.url;
            }
            if (target === "answer") {
              const questionIndex = number(button.dataset.question);
              const answerIndex = number(button.dataset.answer);
              state.quiz.questions[questionIndex].answers[
                answerIndex
              ].content.image_id = attachment.id;
              state.quiz.questions[questionIndex].answers[
                answerIndex
              ].content.image_url = attachment.url;
            }
            if (target === "result") {
              state.quiz.settings.results[index].image_id = attachment.id;
              state.quiz.settings.results[index].image_url = attachment.url;
            }
            if (target === "profile") {
              state.quiz.settings.personality_profiles[index].image_id =
                attachment.id;
              state.quiz.settings.personality_profiles[index].image_url =
                attachment.url;
            }
            markDirty();
            renderBuilder();
    ...
    root.querySelectorAll("[data-remove-media]").forEach(
      (button) =>
        (button.onclick = () => {
          const target = button.dataset.removeMedia;
          const index = number(button.dataset.index);
          if (target === "intro") {
            state.quiz.settings.intro.image_id = 0;
            state.quiz.settings.intro.image_url = "";
          }
          if (target === "question") {
            state.quiz.questions[index].content.image_id = 0;
            state.quiz.questions[index].content.image_url = "";
          }
          if (target === "answer") {
            const questionIndex = number(button.dataset.question);
            const answerIndex = number(button.dataset.answer);
          ...
          if (target === "result") {
            state.quiz.settings.results[index].image_id = 0;
            state.quiz.settings.results[index].image_url = "";
          }
          if (target === "profile") {
            state.quiz.settings.personality_profiles[index].image_id = 0;
            state.quiz.settings.personality_profiles[index].image_url = "";
          }
          markDirty();
          renderBuilder();
  ...
  const chooseMedia = (callback) => {
    if (!window.wp?.media) {
      alert(
        "Η βιβλιοθήκη πολυμέσων του WordPress δεν είναι διαθέσιμη σε αυτή τη σελίδα.",
      );
      return;
    }
  ...
  const refreshPreview = () => {
    root.querySelectorAll("[data-preview-surface]").forEach((surface) => {
      surface.innerHTML = previewMarkup();
    });
    ensureButtonTypes();
    bindPreviewEvents();
  };
  ...
  const save = async (forcedStatus = null, autosave = false) => {
    ...
    if (issues.length) {
      if (!autosave) {
        ...
        toast(
          `Δεν μπορεί να ${publishing ? "δημοσιευτεί" : "αποθηκευτεί"} ακόμη. Διορθώστε τα επισημασμένα σημεία.`,
        );
      }
      return;
    }
    ...
    try {
      const payload = clone(submittedQuiz);
      payload._autosave = autosave;
      if (submittedQuiz.id && submittedQuiz.updated_at)
        payload._expected_updated_at = submittedQuiz.updated_at;
      const savedQuiz = normaliseQuiz(
        await api(requestPath, {
          method: submittedQuiz.id ? "PUT" : "POST",
          body: JSON.stringify(payload),
        }),
      );
      const changedDuringRequest =
        JSON.stringify(state.quiz) !== JSON.stringify(submittedQuiz);
      state.conflict = null;
      state.autosaveFailures = 0;
      /*
      ...
      try {
        localStorage.removeItem(recoveryBeforeSave);
        removeRecovery(state.quiz);
      } catch (_) {}
      if (autosave) {
        const label = root.querySelector(".saved");
      ...
      if (error.status === 409) {
        state.conflict = {
          latest: error.data?.data?.latest || error.data?.latest || null,
        ...
        else if (state.autosaveFailures === 1)
          toast(
            "Η αυτόματη αποθήκευση απέτυχε. Θα ξαναγίνει όταν υπάρχει σύνδεση.",
          );
      }
    } finally {
      state.saving = false;
  ...
  const loadAnalytics = async () => {
    ...
    renderBuilder();
    try {
      state.analytics = await api(
        `quizzes/${state.quiz.id}/analytics?${analyticsQuery()}`,
      );
    } catch (error) {
      ...
      if (state.tab === "analytics") renderBuilder();
    }
  };
  ...
  const loadGlobalAnalytics = async () => {
    ...
    renderGlobalAnalytics();
    try {
      state.dashboardAnalytics = await api(`analytics?${analyticsQuery()}`);
    } catch (error) {
      state.dashboardAnalytics = emptyAnalytics();
      alert(error.message);
    } finally {
      state.loadingPanel = false;
      if (state.view === "analytics-global") renderGlobalAnalytics();
    }
  };
  ...
  const loadRevisions = async () => {
    ...
    renderBuilder();
    try {
      state.revisions = await api(`quizzes/${state.quiz.id}/revisions`);
    } catch (error) {
      state.revisions = [];
      alert(error.message);
    } finally {
      state.loadingPanel = false;
      if (state.tab === "history") renderBuilder();
    }
  };

  window.addEventListener("beforeunload", (event) => {
    if (state.view === "builder" && state.dirty) {
      storeRecovery();
      event.preventDefault();
      event.returnValue = "";
    }
