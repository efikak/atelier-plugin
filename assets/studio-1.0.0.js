...
(() => {
  ...
  `;
    ...
    <button
      type="button"
      class="${active === view ? "is-active" : ""}"
      data-portal-view="${view}"
    >
      <i>${portalIcon(icon)}</i>
      <span>${esc(label)}</span>
    </button>
  `,
    ...
    return `
    <header class="wpqs-portal-topbar${frontClass}">
      ${brand}

      <div
        class="wpqs-portal-nav-shell"
        data-wpqs-scroll-menu
      >
        <button
          type="button"
          class="wpqs-portal-nav-arrow wpqs-portal-nav-prev"
          aria-label="Μετακίνηση menu αριστερά"
        >
          ‹
        </button>

        <div class="wpqs-portal-nav-viewport">
          <nav
            class="wpqs-portal-nav-track"
            aria-label="Κύρια πλοήγηση Quiz Atelier"
          >
            ${buttons}
          </nav>
        </div>
        ...
        <button
          type="button"
          class="wpqs-portal-nav-arrow wpqs-portal-nav-next"
          aria-label="Μετακίνηση menu δεξιά"
        >
          ›
        </button>
      </div>
  ...
  const bindPortalNav = () => {
    ...
    root.querySelectorAll("[data-user-style-open]").forEach((button) => {
  ...
  const renderDocumentation = () => {
    ...
    const query = String(state.docsQuery || "")
      .trim()
      .toLocaleLowerCase("el");
    const rows = documentationArticles.filter(
      (article) =>
        (role === "all" || article.role === "all" || article.role === role) &&
        (!query ||
    ...
    bindPortalNav();
    root.querySelector("[data-doc-search]").oninput = (event) => {
      state.docsQuery = event.target.value;
      renderDocumentation();
      root.querySelector("[data-doc-search]")?.focus();
    };
    root.querySelector("[data-doc-role]").onchange = (event) => {
      state.docsRole = event.target.value;
      renderDocumentation();
    };
