...
(() => {
  ...
  const labels = {
    ...
    continue: "Συνέχεια",
    skip: "Παράλειψη",
    chooseOne: "Επιλέξτε μία απάντηση.",
    chooseAtLeastOne: "Επιλέξτε τουλάχιστον μία απάντηση.",
    enterAnswer: "Γράψτε την απάντησή σας.",
    answerPlaceholder: "Πληκτρολογήστε την απάντησή σας",
    enterNumber: "Γράψτε έναν αριθμό.",
    completeMatching: "Ολοκληρώστε όλες τις αντιστοιχίσεις.",
    noAnswers: "Δεν υπάρχουν διαθέσιμες απαντήσεις.",
    correct: "Σωστά!",
    wrong: "Λάθος απάντηση",
    skipped: "Η ερώτηση παραλείφθηκε",
    pollRecorded: "Η απάντησή σας καταχωρήθηκε.",
    correctAnswer: "Η σωστή απάντηση είναι:",
    correctAnswers: "Οι σωστές απαντήσεις είναι:",
    explanation: "Επεξήγηση",
    timeUp: "Ο χρόνος τελείωσε.",
    calculating: "Υπολογισμός αποτελέσματος…",
    completed: "Ολοκληρώθηκε",
    defaultResult: "Απαντήσατε σωστά σε %1$d από %2$d ερωτήσεις.",
    score: "Βαθμολογία",
    passed: "Επιτυχία",
    failed: "Δεν επιτεύχθηκε η βάση",
    personalityMatch: "Ταίριασμα",
    share: "Κοινοποίηση",
    copyLink: "Αντιγραφή συνδέσμου",
    linkCopied: "Ο σύνδεσμος αντιγράφηκε.",
    restart: "Παίξτε ξανά",
    reviewAnswers: "Δείτε τις απαντήσεις σας",
    hideReview: "Απόκρυψη απαντήσεων",
    unableSubmit: "Δεν ήταν δυνατή η υποβολή",
    tryAgain: "Δοκιμάστε ξανά αργότερα.",
    unavailable: "Το quiz δεν είναι διαθέσιμο αυτή τη στιγμή.",
    expired: "Το quiz έχει λήξει.",
    availableUntil: "Διαθέσιμο έως %s",
    category: "Κατηγορία",
    yourAnswer: "Η απάντησή σας:",
    notAnswered: "Δεν απαντήθηκε",
    correctOrder: "Σωστή σειρά:",
    moveUp: "Πάνω",
    moveDown: "Κάτω",
    blockedTitle: "Μη εγκεκριμένο site",
    blockedMessage:
      "Ωχ! Αυτό το quiz μάλλον πήγε εκδρομή χωρίς άδεια. Το συγκεκριμένο domain δεν βρίσκεται στη λίστα των εγκεκριμένων sites.",
    ...(config.labels || {}),
  };
  ...
  const request = async (path, options = {}) => {
    ...
    try {
      ...
      const error = new Error(
        `Μη έγκυρη απάντηση διακομιστή (${response.status}).`,
      );
    ...
    if (!response.ok) {
      const error = new Error(
        body?.message || `Το αίτημα απέτυχε (${response.status}).`,
      );
      error.status = response.status;
      error.data = body;
      throw error;
  ...
  const normaliseQuiz = (raw) => {
    ...
    quiz.runtime_excluded_questions = Array.isArray(
      quiz.runtime_excluded_questions,
    )
      ? quiz.runtime_excluded_questions.map(Number)
      : [];
    quiz.questions = Array.isArray(quiz.questions)
      ? quiz.questions.map((question) => ({
          ...
          matching_options: Array.isArray(question?.matching_options)
  ...
  const mediaMarkup = (content) => {
    if (content?.image_url)
      return `<img class="wpqs-question-media" src="${esc(content.image_url)}" alt="" loading="lazy">`;
    if (content?.video_url)
      return `<video class="wpqs-question-media" src="${esc(content.video_url)}" controls playsinline></video>`;
    if (content?.audio_url)
      return `<audio class="wpqs-question-media" src="${esc(content.audio_url)}" controls></audio>`;
    return "";
  };
    ...
    const shadow =
      theme.shadow === "none"
        ...
        : theme.shadow === "strong"
          ...
          : "0 12px 42px rgba(15,23,42,.13)";
    return [
      `--wpqs-primary:${esc(theme.primary || "#4f46e5")}`,
      `--wpqs-secondary:${esc(theme.secondary || "#7c3aed")}`,
      `--wpqs-page:${esc(theme.page || "#f4f4f8")}`,
      `--wpqs-background:${esc(theme.background || "#ffffff")}`,
      `--wpqs-text:${esc(theme.text || "#111827")}`,
      `--wpqs-muted:${esc(theme.muted || "#4b5563")}`,
      `--wpqs-button:${esc(theme.button || theme.primary || "#4f46e5")}`,
      `--wpqs-button-text:${esc(theme.button_text || "#ffffff")}`,
      `--wpqs-answer:${esc(theme.answer || "#f8fafc")}`,
      `--wpqs-border:${esc(theme.border || "#cbd5e1")}`,
      `--wpqs-correct:${esc(theme.correct || "#15803d")}`,
      `--wpqs-wrong:${esc(theme.wrong || "#b91c1c")}`,
      `--wpqs-radius:${number(theme.radius, 16)}px`,
      `--wpqs-font:${font}`,
      `--wpqs-shadow:${shadow}`,
    ].join(";");
  };
  const dateLabel = (value) => {
    ...
    try {
      return new Intl.DateTimeFormat(config.locale || "el-GR", {
        dateStyle: "medium",
        timeStyle: "short",
      }).format(date);
    } catch (_) {
  ...
  const guardAllows = () => {
    ...
    return (guard.domains || []).some((raw) => {
      ...
      return (
        parentHost === allowed ||
        (guard.allowSubdomains && parentHost.endsWith(`.${allowed}`))
      );
    });
  };
  const blockedMarkup = () => {
  ...
  if (!guardAllows()) {
    document.querySelectorAll(".wpqs-player").forEach((mount) => {
      mount.innerHTML = blockedMarkup();
    });
    return;
  }
  ...
  document.querySelectorAll(".wpqs-player").forEach(async (mount) => {
    ...
    try {
      ...
      const track = (event, metadata = {}, questionId = 0) =>
        request(`public/quizzes/${id}/events`, {
          method: "POST",
          body: JSON.stringify({
            event,
      ...
      const ruleSatisfied = (rule) => {
        ...
        const target = source.answers.find(
          (answer) =>
            String(answer?.content?.key || "") ===
            String(rule?.answer_key || ""),
        );
      ...
      const conditionSatisfied = (question) => {
        ...
        const rules =
          Array.isArray(condition.rules) && condition.rules.length
            ? condition.rules
            : [
                {
      ...
      const recordTime = (question) => {
        ...
        timings[String(question.id)] = Math.max(
          0,
          Math.round((Date.now() - questionStarted) / 100) / 10,
        );
        questionStarted = 0;
      };
      const moveNext = () => {
      ...
      const optionLabel = (question, optionId) => {
        const item = question.answers.find(
          (answer) => Number(answer.id) === Number(optionId),
        );
        return item
          ? `${item.content?.emoji || ""} ${item.content?.text || ""}`.trim()
          : "";
      };
      ...
      const selectedAnswerMarkup = (question, response) => {
        if (!hasResponse(response))
          return `<div class="wpqs-selected-answer"><strong>${esc(labels.yourAnswer)}</strong><span>${esc(labels.notAnswered)}</span></div>`;
        if (
          ["open_text", "slider", "numeric", "rating"].includes(question.type)
        ) {
        ...
        if (question.type === "matching") {
          const rows = question.answers.map((answer) => {
            const selectedId =
              response?.[String(answer.id)] || response?.[answer.id];
        ...
        const ids = Array.isArray(response)
          ? response.map(Number)
          : [Number(response)];
        const selected = question.answers.filter((answer) =>
          ids.includes(Number(answer.id)),
        );
      ...
      const correctAnswersMarkup = (feedback) => {
        const correct = Array.isArray(feedback?.correct_answers)
          ? feedback.correct_answers
          : [];
        if (!correct.length) return "";
        const heading =
          correct.length > 1 ? labels.correctAnswers : labels.correctAnswer;
      ...
      const emptyResponse = (type) =>
        type === "multiple_answers" || ["ordering", "ranking"].includes(type)
          ? []
          : type === "matching"
            ? {}
            : "";
      const submitQuestion = async (question, response, timedOut = false) => {
        ...
        if (quiz.settings.show_feedback === false) {
          moveNext();
          return;
        }
      ...
      const draw = () => {
        ...
        if (step < 0) {
          ...
          mount.querySelector("button").onclick = () => {
            step = 0;
            track("start");
            draw();
          };
        ...
        while (
          ...
          !conditionSatisfied(quiz.questions[step])
        ) {
        ...
        if (type === "multiple_answers") {
          ...
          const min = number(question.settings?.slider_min, 0),
            max = number(question.settings?.slider_max, 100),
            stepValue = Math.max(
              0.0001,
              number(question.settings?.slider_step, 1),
            );
        ...
        mount.querySelectorAll(".wpqs-answer").forEach(
          (button) =>
            (button.onclick = () => {
              mount
                .querySelectorAll(".wpqs-answer")
        ...
        mount
          .querySelector(".wpqs-slider")
          ?.addEventListener("input", (event) => {
            mount.querySelector("[data-slider-output]").value =
              event.target.value;
          });
        mount.querySelectorAll("[data-rating]").forEach(
          (button) =>
            (button.onclick = () => {
              mount
              ...
              mount.querySelector(".wpqs-rating").dataset.value =
                button.dataset.rating;
            }),
        );
        mount.querySelectorAll("[data-order-move]").forEach(
          (button) =>
            (button.onclick = () => {
        ...
        mount
          .querySelectorAll('.wpqs-order-item[draggable="true"]')
          .forEach((item) => {
            ...
            item.addEventListener("dragend", () => {
              ...
              mount
                .querySelectorAll(".wpqs-order-item")
                .forEach((row, index) => {
                  const count = row.querySelector(".wpqs-order-number");
                  if (count) count.textContent = String(index + 1);
                });
        ...
        mount
          .querySelectorAll("[data-match-left]")
          .forEach((select) =>
            select.addEventListener("change", syncMatchingOptions),
          );
        syncMatchingOptions();
        mount.querySelector(".wpqs-next")?.addEventListener("click", () => {
          ...
          if (type === "multiple_answers") {
            response = [
              ...mount.querySelectorAll('input[type="checkbox"]:checked'),
            ].map((input) => Number(input.value));
            if (required && !response.length) {
              validation.textContent = labels.chooseAtLeastOne;
              return;
            }
          } else if (type === "open_text") {
            response =
              mount.querySelector(".wpqs-open-answer")?.value.trim() || "";
            if (required && !response) {
              validation.textContent = labels.enterAnswer;
              return;
            }
          } else if (type === "slider") {
            ...
            if (required && raw === "") {
              validation.textContent = labels.enterNumber;
              return;
            }
            ...
            if (required && !response) {
              validation.textContent = labels.chooseOne;
              return;
            }
          } else if (["ordering", "ranking"].includes(type)) {
            response = [...mount.querySelectorAll("[data-order-id]")].map(
              (item) => number(item.dataset.orderId),
            );
          } else if (type === "matching") {
            response = {};
            mount.querySelectorAll("[data-match-left]").forEach((select) => {
              if (select.value)
                response[String(select.dataset.matchLeft)] = number(
                  select.value,
                );
            });
            if (
              required &&
              Object.keys(response).length !== question.answers.length
            ) {
              validation.textContent = labels.completeMatching;
              return;
            }
          }
        ...
        mount
          .querySelector(".wpqs-skip")
          ?.addEventListener("click", () =>
            submitQuestion(question, emptyResponse(type)),
          );
      ...
      const finish = async () => {
        ...
        try {
          ...
          const description =
            mapped.description ||
            format(labels.defaultResult, result.correct, result.total);
          const tieProfiles =
            Array.isArray(mapped.tie_profiles) && mapped.tie_profiles.length > 1
              ? `<div class="wpqs-personality-ties">${mapped.tie_profiles.map((profile) => `<span>${esc(profile.title)} ${number(profile.percentage)}%</span>`).join("")}</div>`
              : "";
          const personality = mapped.key
            ? `<p class="wpqs-personality-match">${esc(labels.personalityMatch)}: <strong>${number(mapped.percentage)}%</strong></p>${tieProfiles}`
            : "";
          const pass =
            result.pass === true
              ? `<span class="wpqs-pass is-pass">✓ ${esc(labels.passed)}</span>`
              : result.pass === false
                ? `<span class="wpqs-pass is-fail">× ${esc(labels.failed)}</span>`
                : "";
          const review =
            quiz.settings.review_answers !== false &&
            Array.isArray(result.review) &&
            result.review.length
              ? reviewMarkup(result.review)
              : "";
          ...
          mount
            .querySelector(".wpqs-restart")
            ?.addEventListener("click", restart);
          mount
            .querySelector(".wpqs-review-toggle")
          ...
          const copyLink = async () => {
            const status = mount.querySelector(".wpqs-share-status");
            ...
            try {
              ...
              status.textContent = labels.linkCopied;
              return true;
            } catch (_) {
              try {
              ...
              status.textContent = "Δεν ήταν δυνατή η αντιγραφή.";
              return false;
            }
          };
